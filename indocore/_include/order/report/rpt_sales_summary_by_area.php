...
WHERE " . $strWhere . "
GROUP BY it.icat_midx, TRIM(it.it_code), it.it_model_no, TRIM(cus_city)
ORDER BY it.icat_midx, it_code, cus_city";
...
//echo "1";
if(isset( $group0[$col['icat_midx']][$col['it_code']][$col['cus_city']] )) {
} else {
	$rd[] = array(
		$col['icat_midx'],			//0
		$col['it_code'],	 		//1
	...
	if(isset($qty[$col['it_code']][$col['cus_city']][0])) {
		$qty[$col['it_code']][$col['cus_city']][0] += $col['qty'];
		$qty[$col['it_code']][$col['cus_city']][1] += $col['amount'];
	} else {
		$qty[$col['it_code']][$col['cus_city']][0] = $col['qty'];
		$qty[$col['it_code']][$col['cus_city']][1] = $col['amount'];
	}
	if(empty($item['it_code'])) $item['it_code'] = $col['it_model_no'];
...
END;
	...
	foreach($group1 as $total2 => $group2) {
		...
		foreach($group2 as $total3 => $group3) {
			$i_qty = $qty[$rd[$rdIdx][1]][$rd[$rdIdx][3]];

			if($print_tr_2++ > 0) print "<tr>\n";
			cell(($rd[$rdIdx][3]=='' && strlen($rd[$rdIdx][3])==0)?'<i class="comment">Undefined</i>':$rd[$rdIdx][3]);								// City
			cell(number_format((double)$i_qty[0]), ' align="right"');			// Qty
			cell(number_format((double)$i_qty[1]), ' align="right"');			// Amount before vat
			cell(number_format((double)$i_qty[1]*1/10), ' align="right"');		// vat
			cell(number_format((double)$i_qty[1]*1.1), ' align="right"');		// amount after vat

			$total[0] += $i_qty[0];
			$total[1] += $i_qty[1];
			$total[2] += round($i_qty[1]*1/10);
			$total[3] += round($i_qty[1]*1.1);
