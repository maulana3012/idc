...
/*	2	*/	$pdf->setXY(68,52);
			...
			$pdf->Cell(30, 4, $_code, 0,0,'C');
			$pdf->Cell(27, 4, ($cus[5]=='') ? '' : date('d-M-Y', strtotime($cus['5'])), 0,0,'C');
			$pdf->Cell(40, 4, $_bill_code, 0,0,'C');
/* 	3	*/	$pdf->setXY(13,68);		// Customer to
			$pdf->Cell(95, 4, $_cus_name);
			$pdf->setXY(13,72);
			$pdf->MultiCell(98, 3.5, $_cus_address);
			if($cus[0] != '' && $cus[1] != '') 		{$pdf->setX(13); $pdf->Cell(95, 4, "Telp : ".$cus[0]."  Fax : ".$cus[1]);}
			else if($cus[0] != '' && $cus[1] == '') {$pdf->setX(13); $pdf->Cell(95, 4, "Telp : ".$cus[0]);}
			else if($cus[0] == '' && $cus[1] != '') {$pdf->setX(13); $pdf->Cell(95, 4, "Fax : ".$cus[1]);}
			$pdf->setXY(22,88);
...
while($items =& fetchRow($result,0)) {
	...
	$pdf->Cell(23, 4, number_format($items[5]),0,0,'R');	// amount
	$pdf->Cell(2);
