<strong>[<font color="#446FBE">CUSTOMER</font>] ITEM LIST</strong> &nbsp; <small class="comment"><i><a href="javascript:fillItem()">( search <img src="../../_images/icon/search_mini.gif"> )</a></i></small>
...
<?php while($items =& fetchRow($cusitem_res)) { ?>
		<tr id="<?php echo trim($items[0])?>">
			...
			<td><input type="text" name="_cus_it_unit_price[]" value="<?php echo number_format((double)$items[5])?>" class="reqn" style="width:100%" onBlur="updateAmount()" onKeyUp="formatNumber(this,'dot')"<?php echo ($column['bill_cus_to']=='6IDC') ? " readOnly":"" ?>></td>
			<td><input type="text" name="_cus_it_qty[]" value="<?php echo number_format((double)$items[6]) ?>" style="width:100%" class="reqn" onBlur="updateAmount()" onKeyUp="formatNumber(this,'dot')"></td>
			...
			<td align="center">
				...
				<input type="hidden" name="_cus_it_type[]" value="<?php echo $items[2]?>">
				<a href="javascript:deleteItem('<?php echo trim($items[0])?>')"><img src='../../_images/icon/delete.gif' width='12px'></a>
			</td>
		</tr>
<?php } ?>
	</tbody>
