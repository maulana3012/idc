...
while ($counter[0] < $row[3]) {
	include "generate_billing_idc_invoice_border.php";
	$page[0]++;
}
...
if($_vat_val > 0) {
	if (substr($_code, 0, 2) == "IO") {
		...
		while ($counter[0] < $row[2]) {
			include "generate_billing_idc_pajak_borderless.php";
		}
		// pajak lama < mey 2010
		// include "generate_billing_idc_pajak_borderless_z.php";
...
if($_type_invoice == 0) {
	...
	while ($counter < $row[1]) {
		include "generate_billing_idc_sj_border.php";
		$page[0]++;
	}
	...
	while ($counter < $row[3]) {
		include "generate_billing_idc_ekspedisi_border.php";
		$page[0]++;
	}
}
?>