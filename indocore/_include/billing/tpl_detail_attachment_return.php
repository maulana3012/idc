<?php
$file_sql = "SELECT * FROM ".ZKP_SQL."_tb_return_file WHERE turn_code = '$_code'";
...
<input type="hidden" name="_date" value="<?php echo date("j-M-Y", strtotime($column['turn_return_date']))?>">
...
<table width="100%" class="table_c">
	...
	while($items =& fetchRowAssoc($file_res)) {
	    ...
	    echo "
	    <td align='center'>
		<img  src='". USER_DATA . "archieve" . $items['turnf_file_path'] ."' width='200px'>
		<a target='_blank' href='". USER_DATA . "archieve" . $items['turnf_file_path'] ."'>Download</a> |
		<a href=\"javascript:deleteFile(". $items['turnf_idx'] .", '" . $items['turnf_file_type'].' - '.$items['turnf_file_name'] ."', '".$items['turnf_file_path']."')\">Delete</a><br />
		<strong>[ ". $items['turnf_file_type'] ." ]</strong> ". $items['turnf_file_desc'] ."
	    </td>
	    ";
	    $i++;
	    if( $i == 4 ) { echo "</tr>"; $i=0; }
