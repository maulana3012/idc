...
if ($_cug_code != 'all') {
	...
	$tmp_rt[]	= "rdt_cus_to IN (SELECT cus_code FROM ".ZKP_SQL."_tb_customer WHERE cug_code = '$_cug_code')";
	...
	$sql_rdt	= " SELECT (SELECT cug_name FROM ".ZKP_SQL."_tb_customer_group WHERE cug_code = '$_cug_code') AS cug_name,";
} else {
	$sql_billing = "
	SELECT
...
//include sql
include "generate_sql/generate_delivery_sql_by_item.php";
$sql = "$sql_billing UNION $sql_return UNION $sql_dt UNION $sql_df UNION $sql_dr UNION $sql_rt 
		ORDER BY icat_pidx, icat_midx, it_code, invoice_code";
...
END;
	...
	foreach($group1 as $total2 => $group2) {
		...
		cell('['.trim($rd[$rdIdx][7]). '] '.$rd[$rdIdx][8], ' valign="top" rowspan="'.$rowSpan.'"');
		...
		foreach($group2 as $total3 => $group3) {
			...
			cell_link($rd[$rdIdx][1], ' align="center" valign="top" rowspan="'.$rowSpan.'"',
				' href="'.$rd[$rdIdx][16].'" style="'.$display_css[$rd[$rdIdx][17]].'"');													//invoice no
			cell($rd[$rdIdx][2], ' style="'.$display_css[$rd[$rdIdx][17]].'" align="center" valign="top" rowspan="'.$rowSpan.'"');			//invoice date
			cell($rd[$rdIdx][3], ' style="'.$display_css[$rd[$rdIdx][17]].'" align="center" valign="top" rowspan="'.$rowSpan.'"');			//invoice date
			cell("[".trim($rd[$rdIdx][5])."] ".$rd[$rdIdx][6], ' style="'.$display_css[$rd[$rdIdx][17]].'" valign="top" rowspan="'.$rowSpan.'"');	
...
cell(number_format((double)$grand_total[1]+$grand_total[2]), ' align="right" style="color:brown; background-color:lightyellow"');
print "</tr>\n";
print "</table>\n";
?>