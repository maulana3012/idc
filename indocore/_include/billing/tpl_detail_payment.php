...
function defaultPaymentConfirm() {
	...
	if(f.web_url.value == 'IDC') {
		...
		if(document.frmInsert._ordered_by.value == "1") {
			if(f._method[0].checked || f._method[1].checked || f._method[2].checked) {
				for(i=0; i<2; i++) 
					f._bank[i].disabled = false;
			}
		} else if(document.frmInsert._ordered_by.value == "2") {
			if(f._method[0].checked || f._method[1].checked || f._method[2].checked) {
	...
	if(f._remain_amount.value <= 0) {
		...
		if(f.web_url.value == 'IDC') {
			for(i=0; i<7; i++) {
				f._bank[i].disabled = true; f._bank[i].checked = false;
			}
		} else if(f.web_url.value == 'MED') {
			if(document.frmInsert._ordered_by.value == "1") {
				for(i=0; i<2; i++) {
				...
				f._bank.disabled = true; f._bank.checked = false;
			}
		}
	}
	...
	if (o.checked == true) {
		if(f.web_url.value == 'IDC') {
			...
			if(document.frmInsert._ordered_by.value == "1") {
				...
				if(method == 'cash') {
					for(var i=0; i<2; i++) { f._bank.disabled = true; f._bank.checked = false; }
				} else {
					for(var i=0; i<2; i++) { f._bank.disabled = false; }
				}
			}
		}
	} 
...
window.document.frmPayment.btnSave.onclick = function() {
	...
	if(o.web_url.value == 'IDC') {
		if(o._method[2].checked==true){
			var check = false;
			for (var i=0;i<6;i++) { if(o._bank[i].checked==true) var check = true; }
			if(check == false){
		...
		if(document.frmInsert._ordered_by.value == "1") {
			...
			if(o._method[0].checked==false){
				var check = false;
				if(o._bank.checked==true) var check = true;
				if(check == false){
					alert("Please choose the bank");
					return;
				}
