...
if ($_cug_code != 'all') {
	...
	$tmp_out_month[] = "b.cus_code IN (SELECT cus_code FROM ".ZKP_SQL."_tb_customer WHERE cug_code = '$_cug_code')";
	$tmp_inc_month[] = "b.cus_code IN (SELECT cus_code FROM ".ZKP_SQL."_tb_customer WHERE cug_code = '$_cug_code')";
}
...
$sql = "$sql_out UNION $sql_inc GROUP BY icat_pidx, icat_midx, d.it_code, model, ed, cus_code, cus_full_name ORDER BY icat_pidx, icat_midx, it_code, model, ed, cus_code, cus_full_name";
...
while($col =& fetchRowAssoc($res)) {
	...
	if($cache[2] != $col['ed']) {
		$cache[2] = $col['ed'];
		$group0[$col['icat_pidx'].'-'.$col['icat_midx']][$col['it_code']][$col['ed']] = array();
	}

	if($cache[3] != $col['cus_code']) {
		$cache[3] = $col['cus_code'];
	}

	$group0[$col['icat_pidx'].'-'.$col['icat_midx']][$col['it_code']][$col['ed']][$col['cus_code']] = 1;
}
...
FROM
  ...
  GROUP BY d.it_code, ed, cus_code, v_period
UNION
SELECT
  TRIM(d.it_code) AS it_code,  
  to_char(c.ised_expired_date,'YYYY-MM') AS ed, 
...
GROUP BY d.it_code, ed, cus_code, v_period
ORDER BY it_code, ed, cus_code, v_period, v_column";
$qty = array();
$res_month =& query($sqlQtyperMonth);
while($col =& fetchRowAssoc($res_month)) { 
	if(!isset($qty[$col["it_code"]][$col["ed"]][$col["v_period"]][$col["cus_code"]])) 
		 $qty[$col["it_code"]][$col["ed"]][$col["v_period"]][$col["cus_code"]] = $col["qty"];
	else $qty[$col["it_code"]][$col["ed"]][$col["v_period"]][$col["cus_code"]] += $col["qty"];
}
echo "<pre>";
//echo $sqlQtyperMonth."<BR />";
//var_dump($qty['2101']['2012-03']['201108'], count($qty['2101']['2012-03']['201108']));
//var_dump($qty);
echo "</pre>";
...
END;
	...
	foreach($group1 as $total2 => $group2) {
		...
		foreach($group2 as $total3 => $group3) {
			$rowSpan = 0;
			$rowSpan += count($group3);
			$rowSpan += 1;

			if($print_tr_2++ > 0) print "<tr>\n";
			cell($rd[$rdIdx][4], ' valign="top" align="center" rowspan="'.$rowSpan.'"');	// Expired Date
			...
			print "<tr>\n";
			cell("<b>ED $total3</b>", ' align="right" style="color:darkblue;"');
			for($i=0; $i<$mon_length; $i++) cell(number_format((double)$ed_amount[$i],2), ' align="right" style="color:darkblue;"');
			cell(number_format((double)$ed_amount[$i],2), ' align="right" style="color:darkblue;"');
			cell("", ' align="right" style="color:darkblue;"');
			print "</tr>\n";

			for($i=0; $i<$mon_length; $i++) $it_amount[$i] += $ed_amount[$i];
			$it_amount[$mon_length]	+= $ed_amount[$mon_length];
		}		
		print "<tr>\n";
		cell("<b>$item</b>", ' colspan="2" align="right" style="color:brown; background-color:lightyellow"');
	...
	print "<tr>\n";
	cell("<b>CATEGORY: $cat</b>", ' colspan="3" align="right" style="color:brown; background-color:lightyellow"');
