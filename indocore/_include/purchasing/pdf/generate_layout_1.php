...
/*	8	*/	$pdf->setXY(135, 91);
			$pdf->Cell(33,4,"( ".$curr[$_currency_type]." )",0,0,"C");
			$pdf->Cell(33,4,"( ".$curr[$_currency_type]." )",0,0,"C");
...
if($counter==0) {
	...
	$pdf->setXY(41,100);
	$pdf->Cell(72,4, 'Previous balance');
	$pdf->Cell(20, 3.5, number_format($total[0]), 0,0,'R');
	$pdf->Cell(33);
	$pdf->Cell(33, 3.5, number_format($total[1],2), 0,0,'R');
	$pdf->setY(104);
	$i++;
}
while($items =& fetchRow($result, 0)) {
	...
	$pdf->Cell(5);
	$pdf->Cell(72, 4, substr($items[1],0,32),0);			//ITEM
	$pdf->Cell(20, 4, number_format($items[4]),0,0,'R');	//QTY
	$pdf->Cell(33, 4, number_format($items[3],2),0,0,'R');	//UNIT PRICE
	$pdf->Cell(33, 4, number_format($items[5],2),0,0,'R');	//AMOUNT
	$pdf->Cell(0, 4, '',0,1);
...
$pdf->Cell(33);
$pdf->Cell(33,4, number_format($total[1],2), 0,0,'R');
?>