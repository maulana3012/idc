...
if($_location != 0) {
//    $tmp[0][] = $tmp[1][] = "stk_wh_location = $_location"; 
    $strWhereLoc = "AND log_wh_location = $_location";
} else {
    $strWhereLoc = "";
}
...
if ($_type == 1)
{
	$sql_std = "
        select  
            ...
            END AS dept,
            CASE
                WHEN bill_ordered_by = 1 AND bill_dept = 'A' THEN 'http://192.168.0.88/med/apotik/billing/revise_billing.php?_code='||bill_code
                WHEN bill_ordered_by = 1 AND bill_dept = 'D' THEN 'http://192.168.0.88/med/dealer/billing/revise_billing.php?_code='||bill_code
                WHEN bill_ordered_by = 1 AND bill_dept = 'H' THEN 'http://192.168.0.88/med/hospital/billing/revise_billing.php?_code='||bill_code
                WHEN bill_ordered_by = 1 AND bill_dept = 'M' THEN 'http://192.168.0.88/med/marketing/billing/revise_billing.php?_code='||bill_code
                WHEN bill_ordered_by = 1 AND bill_dept = 'P' THEN 'http://192.168.0.88/med/pharmaceutical/billing/revise_billing.php?_code='||bill_code
                WHEN bill_ordered_by = 1 AND bill_dept = 'T' THEN 'http://192.168.0.88/med/tender/billing/revise_billing.php?_code='||bill_code
            END AS go_page
        from ".ZKP_SQL."_tb_billing where bill_cfm_wh_date is null and bill_inv_date between date '2012-12-01' and date '$period_to' and bill_vat > 0
            UNION
        select  
            turn_code AS doc_no,
            ...
            END AS dept,
            CASE
                WHEN turn_ordered_by = 1 AND turn_dept = 'A' THEN 'http://192.168.0.88/med/apotik/billing/revise_return.php?_code='||turn_code
                WHEN turn_ordered_by = 1 AND turn_dept = 'D' THEN 'http://192.168.0.88/med/dealer/billing/revise_return.php?_code='||turn_code
                WHEN turn_ordered_by = 1 AND turn_dept = 'H' THEN 'http://192.168.0.88/med/hospital/billing/revise_return.php?_code='||turn_code
                WHEN turn_ordered_by = 1 AND turn_dept = 'M' THEN 'http://192.168.0.88/med/marketing/billing/revise_return.php?_code='||turn_code
                WHEN turn_ordered_by = 1 AND turn_dept = 'P' THEN 'http://192.168.0.88/med/pharmaceutical/billing/revise_return.php?_code='||turn_code
                WHEN turn_ordered_by = 1 AND turn_dept = 'T' THEN 'http://192.168.0.88/med/tender/billing/revise_return.php?_code='||turn_code
            END AS go_page
        from ".ZKP_SQL."_tb_return where turn_cfm_wh_delivery_timestamp is null and turn_return_date between date '2012-12-01' and date '$period_to' and turn_vat > 0
            UNION
        select  
            dr_code AS doc_no,
            ...
            END AS dept,
            CASE
                WHEN dr_ordered_by = 1 AND dr_dept = 'A' THEN 'http://192.168.0.88/med/apotik/billing/revise_dr.php?_code='||dr_code
                WHEN dr_ordered_by = 1 AND dr_dept = 'D' THEN 'http://192.168.0.88/med/dealer/billing/revise_dr.php?_code='||dr_code
                WHEN dr_ordered_by = 1 AND dr_dept = 'H' THEN 'http://192.168.0.88/med/hospital/billing/revise_dr.php?_code='||dr_code
                WHEN dr_ordered_by = 1 AND dr_dept = 'M' THEN 'http://192.168.0.88/med/marketing/billing/revise_dr.php?_code='||dr_code
                WHEN dr_ordered_by = 1 AND dr_dept = 'P' THEN 'http://192.168.0.88/med/pharmaceutical/billing/revise_dr.php?_code='||dr_code
                WHEN dr_ordered_by = 1 AND dr_dept = 'T' THEN 'http://192.168.0.88/med/tender/billing/revise_dr.php?_code='||dr_code
            END AS go_page
        from ".ZKP_SQL."_tb_dr where dr_cfm_wh_delivery_timestamp is null and dr_issued_date between date '2012-12-01' and date '$period_to' and dr_type_item = 1
        ORDER BY doc_date";
...
SELECT
  ...
  (SELECT sum(log_in_qty)-sum(log_out_qty) FROM ".ZKP_SQL."_tb_log WHERE it_code = it.it_code AND log_type = 1 $strWhereLoc AND log_date < DATE '$period_from') AS log_start_qty,
  (SELECT sum(log_in_qty) FROM ".ZKP_SQL."_tb_log WHERE it_code = it.it_code AND log_type = 1 $strWhereLoc AND log_date BETWEEN DATE '$period_from' AND '$period_to') AS log_in_qty,
  (SELECT sum(log_out_qty) FROM ".ZKP_SQL."_tb_log WHERE it_code = it.it_code AND log_type = 1 $strWhereLoc AND log_date BETWEEN DATE '$period_from' AND '$period_to') AS log_out_qty,
...
SELECT
  ...
  trim(it_code) AS it_code,
  trim(it_model_no) AS it_model_no,
  (SELECT sum(log_in_qty)-sum(log_out_qty) FROM ".ZKP_SQL."_tb_log WHERE it_code = it.it_code AND log_type = 2 $strWhereLoc AND log_date < DATE '$period_from') AS log_start_qty,
  (SELECT sum(log_in_qty) FROM ".ZKP_SQL."_tb_log WHERE it_code = it.it_code AND log_type = 2 $strWhereLoc AND log_date BETWEEN DATE '$period_from' AND '$period_to') AS log_in_qty,
  (SELECT sum(log_out_qty) FROM ".ZKP_SQL."_tb_log WHERE it_code = it.it_code AND log_type = 2 $strWhereLoc AND log_date BETWEEN DATE '$period_from' AND '$period_to') AS log_out_qty,
  (SELECT ipn_price_dollar FROM ".ZKP_SQL."_tb_item_price_net WHERE it_code = it.it_code AND ipn_date_from <= '$period_to' AND ipn_idx = (SELECT max(ipn_idx) FROM ".ZKP_SQL."_tb_item_price_net WHERE it_code = it.it_code AND ipn_date_from <= '$period_to')) AS ipn_price_dollar,
  ".ZKP_SQL."_getuserprice(it_code, DATE '$period_to', 'net', ".str_replace(',','',$_kurs).") AS ipn_price
FROM
  ".ZKP_SQL."_tb_item AS it
...
/*
echo "<pre>";
var_dump($_location, $sql);
echo "</pre>";
exit;
*/
if($currentDept == 'report_all')
{
