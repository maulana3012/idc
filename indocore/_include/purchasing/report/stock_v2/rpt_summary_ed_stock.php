...
{
	if($_location != 0) {
		$tmp[]   = "e.sted_wh_location = $_location"; 
	}
}
...
SELECT
  ...
  it.it_model_no,
  e.sted_expired_date AS expired_date,
  ".ZKP_SQL."_getRemainMonth(e.sted_expired_date) AS remain_month,
  SUM(e.sted_qty) AS total_qty
FROM
  ".ZKP_SQL."_tb_stock_ed AS e
  JOIN ".ZKP_SQL."_tb_item AS it USING(it_code)
  JOIN ".ZKP_SQL."_tb_item_cat AS icat USING(icat_midx)
WHERE ". $strWhere ."
GROUP BY icat.icat_pidx, icat.icat_midx, it.it_code, it.it_model_no, e.sted_expired_date
...
END;
	...
	foreach($group1 as $total2 => $group2) {
		...
		foreach($group2 as $total3 => $group3) {
			...
			if($status=='expired' && $currentDept == 'warehouse') {
...
print <<<END
	<table width="60%" class="table_box">
		<tr>
			<th width="15%">MODEL NO</th>
			<th width="10%">EXPIRED DATE</th>
			<th width="5%">M/S<br />(month/s)</th>
