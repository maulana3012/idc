...
//echo $errNo;
	...
	$ma_idx = $result[0];
	$ma_group = array(
					0=>array('ALL', 'IDC', 'MED', 'MEP', 'SMD'),
					1=>array(100,101,102,103,104,105,106,107,111,112,113,114,115,116,117,130,131,132,133,134,135,136)
			   );
	...
	$S->setValue("ma_is_manager_smd", $row['ma_is_manager_smd']);	
	...
	$sql = "SELECT 'ma_' || gr_access ||'_'|| gr_idx AS group, gm_perm AS perm from tb_gmember where gm_perm > 0 AND ma_idx=".$ma_idx;
	$res =& query($sql);
	while($col =& fetchRowAssoc($res)) {
		$perm[$col['group']] = $col['perm'];
	}

	// SET ACCOUNT PERM FOR EACH MODUL
	for($i=0; $i<count($ma_group[0]); $i++) {
		for($j=0; $j<count($ma_group[1]); $j++) {
			$a = "ma_".strtoupper($ma_group[0][$i])."_".$ma_group[1][$j];
			$S->setValue($a, empty($perm[$a]) ? 0 : $perm[$a]);
		}
	}
