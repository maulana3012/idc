...
SELECT 
	...
	ipn_price_kurs, 
	ipn_price_dollar,
	ipn_price_kurs*ipn_price_dollar AS ipn_price_rupiah,
	ipn_updated_by, 
	ipn_updated
...
function calAmount(part) {
	if(part == 'add') {
		var b1 = removecomma(window.document.frmInsertPriceNet._price_dollar.value);
		var b2 = removecomma(window.document.frmInsertPriceNet._price_kurs.value);
		window.document.frmInsertPriceNet._price_rupiah.value = addcomma(b1*b2);
	} else if (part == 'update') {
		var b1 = removecomma(window.document.frmUpdatePriceNet._price_dollar.value);
		var b2 = removecomma(window.document.frmUpdatePriceNet._price_kurs.value);
		window.document.frmUpdatePriceNet._price_rupiah.value = addcomma(b1*b2);
	}
}
</script>
</head>
...
<input type="hidden" name="_code" value="<?php echo $column['it_code']?>">
	<tr>
		...
		<th width="8%">PRICE ($)</th>
		<th width="8%">KURS</th>
		<th width="12%">PRICE (Rp)</th>
		<th colspan="2">REMARK</th>
	</tr>
	<tr>
		<td align="center"><input type="text" name="_date_from" class="reqd" size="10"></td>
        <td></td>
		<td align="right"><input type="text" name="_price_dollar" class="reqn" value="0" style="width:100%" onKeyUp="formatNumber(this, 'dot')" onBlur="calAmount('add')"></td>
		<td align="right"><input type="text" name="_price_kurs" class="reqn" value="0" style="width:100%" onKeyUp="formatNumber(this, 'dot')" onBlur="calAmount('add')"></td>
		<td align="right"><input type="text" name="_price_rupiah" class="reqn" value="0" style="width:100%" onKeyUp="formatNumber(this, 'dot')" readOnly></td>
...
while($col =& fetchRowAssoc($res_price_net)) {
	...
	if ($col['ipn_date_to'] == '') {
		...
		cell('<input type="text" name="_price_dollar" class="reqn"  style="width:100%" value="'.number_format($col['ipn_price_dollar'],2).'" onKeyUp="formatNumber(this,\'dot\')" onBlur="calAmount(\'update\')">', ' align="right"');
		cell('<input type="text" name="_price_kurs" class="reqn"  style="width:100%" value="'.number_format($col['ipn_price_kurs']).'" onKeyUp="formatNumber(this,\'dot\')" onBlur="calAmount(\'update\')">', ' align="right"');
		cell('<input type="text" name="_price_rupiah" class="reqn"  style="width:100%" value="'.number_format($col['ipn_price_rupiah']).'" readOnly>', ' align="right"');
		...
		cell(number_format((double)$col['ipn_price_kurs']), ' align="right"');
		cell(number_format((double)$col['ipn_price_rupiah']), ' align="right"');
...
<?php while($col =& fetchRowAssoc($res_price_net)) { ?>
	<tr>
        <td align="center"><?php echo $col['ipn_date_from'] ?></td>
        <td align="center"><?php echo ($col['ipn_date_to'] == '') ? '-' : $col['ipn_date_to'] ?></td>
        <td align="right"><?php echo number_format($col['ipn_price_dollar'],2) ?></td>
        <td align="right"><?php echo number_format($col['ipn_price_kurs']) ?></td>
        <td align="right"><?php echo number_format($col['ipn_price_rupiah']) ?></td>
        <td><?php echo $col['ipn_remark'] ?></td>
	</tr>
<?php } ?>
</table><br />
<?php } ?>
