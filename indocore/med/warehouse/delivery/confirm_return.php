...
if($column['std_doc_type'] == 1)  {
	$cus_sql = "SELECT a.it_code, a.it_model_no, a.it_desc, reit_qty, reit_remark FROM ".ZKP_SQL."_tb_return JOIN ".ZKP_SQL."_tb_return_item USING (turn_code) JOIN ".ZKP_SQL."_tb_item AS a USING(it_code) WHERE turn_code = '".trim($column['std_doc_ref'])."'";	
} else if($column['std_doc_type'] == 2)  {
	$cus_sql = "SELECT a.it_code, a.it_model_no, a.it_desc, roit_qty, roit_remark FROM ".ZKP_SQL."_tb_return_order JOIN ".ZKP_SQL."_tb_return_order_item USING (reor_code) JOIN ".ZKP_SQL."_tb_item AS a USING(it_code) WHERE reor_code = '".trim($column['std_doc_ref'])."'";
} else if($column['std_doc_type'] == 3)  {
	$cus_sql = "SELECT a.it_code, a.it_model_no, a.it_desc, rdtit_qty, rdtit_remark FROM ".ZKP_SQL."_tb_return_dt JOIN ".ZKP_SQL."_tb_return_dt_item USING (rdt_code) JOIN ".ZKP_SQL."_tb_item AS a USING(it_code) WHERE rdt_code = '".trim($column['std_doc_ref'])."'";
}
...
<body topmargin="0" leftmargin="0" onLoad="window.document.all.btnConfirm.focus();">
...
<input type="hidden" name="_revision_time" value="<?php echo $column['std_revision_time']?>">
	<table width="100%" class="table_box">
		<tr>
			<td colspan="4"><span class="bar_bl">RETURN INFORMATION</span></td>
...
<script language="javascript" type="text/javascript">
	...
	window.document.all.btnConfirm.onclick = function() {
		...
		var idx_stock	= idx_code+4;
		var idx_demo	= idx_code+5;
		var idx_reject	= idx_code+6;
		...
		for (var i=0; i<row; i++) {
			...
			for (var l=0; l<rowIII; l++) {
				if(code == trim(e(idx_codeIII+l*numInputIII).value)) {
					qtyIII	= qtyIII + 1;
				}
			}
