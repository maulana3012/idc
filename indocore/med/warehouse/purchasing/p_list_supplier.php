...
$sqlQuery = new strSelect("SELECT sp_code, sp_full_name, sp_contact_name, sp_phone, sp_fax, sp_address  FROM ".ZKP_SQL."_tb_supplier_local");
...
while ($rows =& fetchRow($result,0)) {
	printf("sp['%s']=['%s','%s','%s','%s','%s','%s'];\n",
		...
		addslashes($rows[2]),	//Attn
		addslashes($rows[3]),	//Phone
		addslashes($rows[4]),	//Fax
		preg_replace($ptn, $rpm, $rows[5]) //Address 
	);
}
?>
...
function fillField(idx) {
	...
	f._sp_attn.value	= sp[idx][2];
	f._sp_phone.value 	= sp[idx][3];
	f._sp_fax.value		= sp[idx][4];
	f._sp_address.value	= sp[idx][5];
...
?>
	<tr>
		...
		<td width="15%"><a href="javascript:fillField('<?php echo addslashes(html_entity_decode($column['sp_code']))?>')" style="color:darkblue"><?php echo $column['sp_code']?></a></td>
