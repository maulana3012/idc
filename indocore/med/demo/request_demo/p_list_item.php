...
function createNewItem() {
	...
	if (f._wh_it_code.value.length <= 0) {
		...
		alert("Qty value can't more than stock value");
		f._wh_it_qty.value = addcomma(f._est_stock.value);
		f._wh_it_qty.focus();
		return;
	} 

	f._wh_it_qty.value 	 	= parseFloat(removecomma(f._wh_it_qty.value));
...
<small>* Printed for move stock to demo stock</small>
</strong>
<hr>

<form name="frmSearch" id="frmSearch">
<input type="hidden" name="_dept" value="z">
...
<form name="frmCreateItem" method="POST" action="./p_list_item.php">
