...
if($_order_by == 1){
	...
	$tmpbill_item[] = "bill.bill_ordered_by = 1";
	$tmpturn_item[] = "turn.turn_ordered_by = 1";
} else if($_order_by == 2) {
	$tmp_bill[]	= "bill_ordered_by = 2";
	$tmp_turn[] = "turn_ordered_by = 2";
	$tmp_dr[]	= "dr_code is null";
	$tmpbill_item[] = "bill.bill_ordered_by = 2";
	$tmpturn_item[] = "turn.turn_ordered_by = 2";
...
if($_vat == 'vat') {
	...
	$tmpbill_item[] = "bill.bill_vat > 0";
	$tmpturn_item[] = "turn.turn_vat > 0"; 
	$tmpdr_item[]	= "dr_type_item = 1";
} else if($_vat == 'vat-IO') {
	$tmp_bill[] 	= "bill.bill_vat > 0 AND bill.bill_type_pajak = 'IO'";
	$tmp_turn[] 	= "turn.turn_vat > 0";
	$tmp_dr[]		= "dr_type_item = 1";
	$tmpbill_item[] = "bill.bill_vat > 0 AND bill.bill_type_pajak = 'IO'";
	$tmpturn_item[] = "turn.turn_vat > 0";
	$tmpdr_item[]	= "dr_type_item = 1";
} else if($_vat == 'vat-IP') {
	$tmp_bill[] 	= "bill.bill_vat > 0 AND bill.bill_type_pajak = 'IP'";
	$tmp_turn[] 	= "turn.turn_code = NULL";
	$tmp_dr[]		= "dr_code is null";
	$tmpbill_item[] = "bill.bill_vat > 0 AND bill.bill_type_pajak = 'IP'";
	$tmpturn_item[] = "turn.turn_code = NULL";
	$tmpdr_item[]	= "dr_code is null";
} else if ($_vat == 'non') {
	$tmp_bill[] 	= "bill.bill_vat = 0";
...
END;
	...
	foreach($group1 as $total2 => $group2) {
		...
		cell(number_format($total[0]), ' valign="top" align="right"');	//qty
		cell(number_format($total[1]), ' valign="top" align="right"');	//amount
		cell(number_format($rate, 2)." %", ' valign="top" align="right"');
		if($a != $rd[$rdIdx][0]) {
			cell(number_format($sub_rate, 2)." %", ' rowspan="'. $rowSpan .'" align="right" style="color:darkblue"');
			$a = $rd[$rdIdx][0];
		}
		print "</tr>\n";
...
<table width="100%" class="table_f">
	<tr>
			<th>MODEL NO</th>
			<th width="15%">QTY<br>(EA)</th>
			<th width="20%">AMOUNT (Rp.)<br>(Qty * (Unit Price* (100-disc)/100))</th>
			<th width="10%">RATE</th>
			<th width="10%">RATE</th>
	</tr>\n
END;
