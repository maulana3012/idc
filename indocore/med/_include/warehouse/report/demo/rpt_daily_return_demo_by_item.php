...
SELECT
  ...
  'detail_request.php?_code=' || use_code AS go_page_request
FROM
...
while($col =& fetchRowAssoc($res)) {
	...
	$rd[] = array(
		...
		$col['cus_full_name'],	//9
		$col['rdit_qty'],		//10
		$col['go_page_return'],	//11
		$col['go_page_request']	//12
	);
...
END;
	...
	foreach($group1 as $total2 => $group2) {
		...
		foreach($group2 as $total3 => $group3) {
			...
			cell($rd[$rdIdx][7], ' valign="top" align="center"');					//Confirm date
			cell("[".trim($rd[$rdIdx][8])."] ".$rd[$rdIdx][9], ' valign="top"');	//Customer / Event
