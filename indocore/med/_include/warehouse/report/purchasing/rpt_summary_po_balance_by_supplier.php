...
SELECT
 ...
 (select sum(init_qty) from ".ZKP_SQL."_tb_in_local join ".ZKP_SQL."_tb_in_local_item using(inlc_idx) where po_code=po.po_code and it_code=it.it_code) AS in_qty,
 '# '||".ZKP_SQL."_poLocallastArrived('pl', po_code, it_code) AS last_arrived_inv,
 ".ZKP_SQL."_poLocallastArrived('date', po_code, it_code) AS last_arrived_date,
...
END;
	...
	foreach($group1 as $total2 => $group2) {
		...
		foreach($group2 as $group3) {
			...
			cell(number_format($rd[$rdIdx][8]), ' style="'.$display_css[$rd[$rdIdx][4]].'" align="right"');		//RECEIVED QTY
