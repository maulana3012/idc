...
while($col =& fetchRowAssoc($res)) {
	...
	$td[] = array(
		...
		$col['cus_code'],
		$col['cus_full_name'],
		cut_string($col['ap_desc'], 20),
		$col['date_from'],
		$col['date_to'],
		$col['ap_basic_disc_pct'],
		$col['ap_disc_pct'],
		$col['ap_is_valid'],
		$col['ap_is_apply_all']);
...
for ($i = 0; $i < $numItem; $i++) {
	...
	for ($o = 0; $o < $rowSpan[$i]; $o++) {
		...
		cell_link('['.$td[$rowIdx][0].'] '.$td[$rowIdx][3], '', " href=detail_apotik_policy.php?_code=".$td[$rowIdx][0]);
		...
		if($td[$rowIdx][9] == 'f') {
	 		cell_link("VIEW", ' align="center"', ' href="javascript:openWindow(\'./p_apotik_disc_item_list.php?_idx='.$td[$rowIdx][0].'\', 700, 350)"');
		} else {
