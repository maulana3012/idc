<?php
$pdf->setSourceFile(APP_DIR . "_include/order/template_pdf/" . $tpl_pdf[4]);
...
//Head Total
$i = 0;
pg_result_seek($res_freight, $counter);
if($counter == 0) {
	$pdf->setY(84);
} else {
	$pdf->setXY(10, 84);
	$pdf->Cell(120,4, 'Previous balance',0);
...
while($items =& fetchRow($res_freight, 0)) {
	...
	$pdf->Cell(70, 3.5, substr($items[2],0,38),0);					// description
...
/*	7	*/	$pdf->setXY(165,269);
			$pdf->Cell(30, 4, $_sign_by, 0, 0, 'C');					// Sign by
