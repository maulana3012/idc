...
//if($_code == 'BO-00035A-A12' || $_code == 'BO-00036A-A12') {
//	$_revision_time -=  1 ;
//} else {
	$result = executeSP(
...
if(ckperm(ZKP_INSERT, HTTP_DIR . "$currentDept/$moduleDept/index.php", 'upload_file')) {
	...
	$valid_type = array ('image/x-png','image/jpeg','image/gif', 'image/jpg', 'image/pjpeg');
	...
	if (isZKError($result))
	{
		$M->goErrorPage($result, "revise_billing.php?_code=".$_code);
	}

	$M->goPage("revise_billing.php?_code=".$_code);
...
if(ckperm(ZKP_DELETE, HTTP_DIR . "$currentDept/$moduleDept/index.php", 'upload_file_delete')) {
	...
	$sql = "DELETE FROM med_tb_billing_file WHERE bill_code='$_code' AND billf_idx=$_idx ";
 
