...
$row	= array(22, 0, $row_cus, 0, 0); //0. default limit, 1. adding row, 2.jumlah item, 3.jumlah total baris, 4.-
if($_vat_val>0) {
	if($_disc>0) $row[1]=4;
	else		 $row[1]=2;
} else {
...
while ($counter[0] < $row[3]) {
	if($_dept == 'A' || $_dept == 'T') {
		include "generate_billing_med_invoice_border.php";
	} else {
		include "generate_billing_med_invoice_borderless.php";
	}
	$page[0]++;
}
...
if($_vat_val > 0) {
	if (substr($_code, 0, 2) == "BO") {
		...
		while ($counter[0] < $row[2]) {
			include "generate_billing_med_pajak_borderless.php";
		}
		/*
		// PAJAK BORDER
		$counter= array(0,1);					// counter, no item
		...
		while ($counter[0] < $row[2]) {
			include "generate_billing_pajak_manual.php";
		}
		$counter= array(0,1);					// counter, no item
		$total	= array(0,0,0);					// row 1, 2, 4
		$row	= array(15, 0, $row_cus);		//0. default limit, 1. adding row, 2.jumlah item, 3.jumlah total baris, 4.-
		...
		*/
		...
		while ($counter[0] < $row[2]) {
			include "generate_billing_pajak_sederhana.php";
			$page[0]++;
		}
	}
}
...
if($_type_invoice == 0) {
	...
	while ($counter[$big] < $row[$big+2]) {
		include "generate_billing_warehouse.php";
		$page[0]++;
	}
	...
	$counter = 0;
	$qty = 0;
	$row = array(22, $row_cus); //0.limit, 1.total item
	$page = array(1, ceil($row[1] / $row[0]));
	$title = "SURAT JALAN";
	while ($counter < $row[1]) {
		if($_dept == 'A' || $_dept == 'T') {
			include "generate_billing_med_sj_border.php";
		} else {
			include "generate_billing_med_sj_borderless.php";
		}
		$page[0]++;
	}
	...
	$row[4] = $row[0]-$row[1];
	$page = array(1, ceil($row[3] / $row[0]));
	$title = "INVOICE INSURANCE COURIER SERVICE";
	while ($counter < $row[3]) {
		include "generate_billing_med_ekspedisi_borderless.php";
//		include "generate_billing_med_ekspedisi_border.php";
		$page[0]++;
	}
}
?>