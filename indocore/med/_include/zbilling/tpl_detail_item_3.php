...
$slitem_sql = "
SELECT
	to_char(bisl_date, 'dd-Mon-yy') AS date,
	cus_code,
	cus_full_name,
	bisl_sl_faktur_no,
	bisl_lop_no,
...
FROM
  ...
  JOIN ".ZKP_SQL."_tb_billing_sales as s using(cus_code)
WHERE bill_code = '$_code'
ORDER BY bisl_date, cus_code";
$slitem_res	=& query($slitem_sql);
?>
...
<?php 
$_id = "";
while($items =& fetchRow($slitem_res)) { 
	for($i=0; $i<6; $i++) $_id = $_id ."|||". trim($items[$i]);
?>
	<tr id="<?php echo $_id ?>">
		<td align="center"><input type="hidden" name="_sl_date[]" value="<?php echo $items[0]?>"><?php echo $items[0]?></td>
		<td align="center"><input type="hidden" name="_sl_cus_code[]" value="<?php echo $items[1]?>"><?php echo $items[1]?></td>
		<td><input type="hidden" name="_sl_cus_name[]" value="<?php echo $items[2]?>"><?php echo $items[2]?></td>
		<td><input type="hidden" name="_sl_faktur_no[]" value="<?php echo $items[3]?>"><?php echo $items[3]?></td>
		<td><input type="hidden" name="_sl_lop_no[]" value="<?php echo $items[4]?>"><?php echo $items[4]?></td>
		<td align="right"><input type="text" name="_sl_amount[]" class="fmtn" style="width:100%" value="<?php echo number_format((double)$items[5])?>" readonly></td>
		<td align="center"><a href="javascript:deleteSales('<?php echo $_id ?>')"><img src='../../_images/icon/delete.gif' width='12px'></a></td>
	</tr>
<?php 
	$_id = "";
}
...
<?php while($items =& fetchRow($cusitem_res)) { ?>
		<tr id="<?php echo trim($items[0]).number_format((double)$items[5]) ?>">
			<td><input type="hidden" name="_cus_it_code[]" value="<?php echo $items[0] ?>"><?php echo $items[0] ?></td>
			<td><input type="text" name="_cus_it_model_no[]" class="fmt" style="width:100%" value="<?php echo $items[2] ?>"></td>
			<td><input type="text" name="_cus_it_desc[]" class="fmt" style="width:100%" value="<?php echo $items[4] ?>"></td>
			<td><input type="text" name="_cus_it_unit_price[]" class="fmtn" style="width:100%" value="<?php echo number_format((double)$items[5]) ?>"></td>
			<td><input type="text" name="_cus_it_qty[]" class="fmtn" style="width:100%" value="<?php echo number_format((double)$items[6]) ?>"></td>
			<td><input type="text" name="_cus_it_amount[]" class="fmtn" style="width:100%" value="<?php echo number_format((double)$items[7]) ?>"></td>
			<td><input type="text" name="_cus_it_remark[]" class="fmt" style="width:100%" value="<?php echo $items[8] ?>"></td>
			<td><input type="hidden" name="_cus_it_sl_idx[]" value="<?php echo $items[9] ?>"></td>
		</tr>
<?php } ?>
-->
	</tbody>
...
function checkUpdatedSalesAmount() {
	...
	for (var i=0; i<count; i++) {
		...
		var col = new Array;
		col[0] = trim(oRow.cells(0).innerText);	// date
		col[1] = trim(oRow.cells(1).innerText);	// cus code
		col[2] = trim(oRow.cells(3).innerText);	// faktur no
		col[3] = trim(oRow.cells(4).innerText);	// LOP no
		col[4] = parseInt(removecomma(e(idx_sl_amount+(i*numInput)).value));	// Amount

		var amount = 0;
		for (var j=0; j<sl.length; j++) {
			if(col[0]==sl[j][4] && col[1]==sl[j][2] && col[2]==sl[j][12] && col[3]==sl[j][13]) {
				amount = amount + parseInt(removecomma(sl[j][11]));
		...
		e(idx_sl_amount+(i*numInput)).value	= numFormatval(amount+'',0);;
	}

	if(isdifferent) {
		alert("One or more sales log in this invoice already changed. \nPlease update the invoice, to automatically change it same with the sales");
		window.document.all.btnUpdate.focus();
	}
...
function createItem() {
	...
	for(var i=0; i<f2.length; i+=7) {
		...
		for(var j=0; j<7; j++) {
			...
			var id = ''; for(var k=0; k<6; k++) { id = id +'|||'+ trim(f2.elements[i+k].value); }
...
function updateQtyBill(indicator, code, price, sl_idx) {
	...
	var numInputI	= 6;	/////
	var numInputII	= 8;	/////
	var idx_price	= 45+(numInputI*countI);	/////
