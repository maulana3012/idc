...
$sql	= "
SELECT *, 
  (SELECT std_idx FROM ".ZKP_SQL."_tb_outstanding WHERE std_doc_ref='$_code' AND std_doc_type=3) AS std_idx,
  (SELECT inc_idx FROM ".ZKP_SQL."_tb_incoming WHERE inc_doc_ref='$_code' AND inc_doc_type=3) AS inc_idx
...
if(numQueryRows($result) <= 0) {
	...
	goPage(HTTP_DIR . "$currentDept/$moduleDept/detail_return_dt.php?_code=".urlencode($column['rdt_code']));
}
  ...
  <tr>
    <td style="padding:0 3 3 3">
        ...
        <tr>
          	...
          	<!--START: BODY-->
<strong class="title">[<font color="#446fbe"><?php echo strtoupper($currentDept) ?></font>] REVISE RETURN DT</strong><br /><br />
<form name='frmInsert' method='POST'>
<input type='hidden' name='p_mode' value="insert">
<input type='hidden' name='_code' value="<?php echo $column["rdt_code"] ?>">
<input type='hidden' name='_revision_time' value="<?php echo $column["rdt_revesion_time"] ?>">
...
<input type='hidden' name='_dt_date' value="<?php echo $column['rdt_dt_date'] ?>">
...
<table width="100%" class="table_box">
	<tr>
		...
		<td><input type="text" name="_date" class="reqd" size="15" value="<?php echo date('j-M-Y', strtotime($column['rdt_date'])) ?>"></td>
	</tr>
	<tr>
		<th>ISSUED BY</th>
		<td><input name="_issued_by" type="text" class="req" size="15" maxlength="32" value="<?php echo $column["rdt_issued_by"] ?>"></td>
		<th>TYPE INVOICE</th>
		<td>
...
<table width="100%" class="table_box" cellspacing="1">
	<tr>
		...
		<td width="20%"><input type="text" name="_cus_to" class="req" size="5" value="<?php echo $column['rdt_cus_to'] ?>"></td>
		<th width="12%">NAME</th>
		<td><input type="text" name="_cus_name" class="fmt" style="width:100%" value="<?php echo $column['rdt_cus_name'] ?>"></td>
	</tr>
	<tr>
		<th>ADDRESS</th>
		<td colspan="3"><input type="text" name="_cus_address" class="fmt"  style="width:100%" maxlength="255" value="<?php echo $column['rdt_cus_address'] ?>"></td>
	</tr>
	<tr>
		<th>SHIP TO</th>
		<th><a href="javascript:fillCustomer('ship')">CODE</a></th>
		<td><input type="text" name="_ship_to" class="req" size="5" value="<?php echo $column['rdt_ship_to'] ?>"></td>
		<th>NAME</th>
		<td><input type="text" name="_ship_name" class="fmt" style="width:100%" value="<?php echo $column['rdt_ship_name'] ?>"></td>
	</tr>
	<tr>
		<th>DT NO</th>
		<th>CODE</th>
...
<table class="table_box" width="100%">
	<tr>
		...
		<td>1.<input type="text" name="_delivery_warehouse" class="fmt" size="2" maxlength="2" value="<?php echo $column["rdt_delivery_warehouse"] ?>">ex W/house(P/C/D)</td>
		<td>2.<input type="text" name="_delivery_franco" class="fmt" size="2" maxlength="2" value="<?php echo $column["rdt_delivery_franco"] ?>">Franco(P/D)</td>
		<td>by <input type="text" name="_delivery_by" size="6" class="fmt" value="<?php echo $column["rdt_delivery_by"] ?>"></td>
		<td>Freight charge : Rp <input type="text" name="_delivery_freight_charge" class="fmtn" value="<?php echo number_format((double)$column["rdt_delivery_freight_charge"]) ?>" onKeyUp="formatNumber(this,'dot')" size="8" onBlur="updateAmount()"></td>
	</tr>
	<tr>
		<th>REMARK</th>
		<td colspan="4"><textarea name="_remark" style="width:100%" rows="4"><?php echo $column["rdt_remark"] ?></textarea></td>
	</tr>
</table>
</form>
