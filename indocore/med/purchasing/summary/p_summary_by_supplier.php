...
$variable = array(
	'order'		=> array('0'=>'==ALL==', '1'=>'INDOCORE', '2'=>'MEDIKUS EKA'),
...
<table class="table_box" width="100%">
	...
	<tr>
		...
		<th>VAT</th>
		<td><?php echo $variable['doc'][$_vat] ?></td>
	</tr>
</table><br />
<?php
$tmp_bill	= array();
...
foreach($icat_name as $key => $value) {
	...
	if($qty!=0 || $amount!=0 || $rate!=0) {
		print "<tr>\n";
		cell($value);
		cell(number_format($qty), ' align="right"');
		cell(number_format($amount), ' align="right"');
		cell(number_format($rate,2)."%", ' align="right"');
		print "</tr>\n";
	}
...
cell(number_format($total[0]), ' align="right" style="color:brown; background-color:lightyellow"');
cell(number_format($total[1]), ' align="right" style="color:brown; background-color:lightyellow"');
cell(number_format($total[2],2)."%", ' align="right" style="color:brown; background-color:lightyellow"');
print "</tr>\n";
print "</table>";
?>
</body>
