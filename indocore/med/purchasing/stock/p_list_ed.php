...
$strWhere = array(
/*0*/	0=>	"it_code='$_code' AND eni_type=$_type AND eni_wh_location=$_loc AND eni_expired_date=date '$_date'".		(isset($_filter_act[0]) ? $_filter_act[0] : ''),
/*1*/	1=>	"it_code='$_code' AND epl_type=$_type AND epl_wh_location=$_loc AND epl_expired_date=date '$_date'".		(isset($_filter_act[1]) ? $_filter_act[1] : ''),
/*2*/	2=>	"it_code='$_code' AND ecl_type=$_type AND ecl_wh_location=$_loc AND ecl_expired_date=date '$_date'".		(isset($_filter_act[2]) ? $_filter_act[2] : ''),
/*3*/	3=>	"it_code='$_code' AND elc_type=$_type AND elc_wh_location=$_loc AND elc_expired_date=date '$_date'".		(isset($_filter_act[3]) ? $_filter_act[3] : ''),
/*4*/	4=>	"it_code='$_code' AND ised_type=$_type AND ised_wh_location=$_loc AND ised_expired_date=date '$_date'".		(isset($_filter_act[4]) ? $_filter_act[4] : ''),
/*5*/	5=>	"eed.it_code='$_code' AND eed_type=$_type AND eed_wh_location=$_loc AND eed_expired_date=date '$_date'".	(isset($_filter_act[5]) ? $_filter_act[5] : ''),
/*6*/	6=>	"it_code='$_code' AND rebed_to_type=$_type AND rebed_to_wh=$_loc AND rebed_expired_date=date '$_date'".		(isset($_filter_act[6]) ? $_filter_act[6] : ''),
/*7*/	7=>	"it_code='$_code' AND mved_to_type=$_type AND mved_to_wh=$_loc AND mved_expired_date=date '$_date'".		(isset($_filter_act[7]) ? $_filter_act[7] : ''),
/*8*/	8=>	"it_code='$_code' AND oted_type=$_type AND oted_wh_location=$_loc AND oted_date=date '$_date'".				(isset($_filter_act[8]) ? $_filter_act[8] : ''),
/*9*/	9=>	"it_code='$_code' AND bed_from_type=$_type AND bed_from_wh=$_loc AND bed_expired_date=date '$_date'".		(isset($_filter_act[9]) ? $_filter_act[9] : ''),
/*10*/	10=>"it_code='$_code' AND rebed_from_type=$_type AND rebed_from_wh=$_loc AND rebed_expired_date=date '$_date'".	(isset($_filter_act[10]) ? $_filter_act[10] : ''),
/*11*/	11=>"it_code='$_code' AND mved_from_type=$_type AND mved_from_wh=$_loc AND mved_expired_date=date '$_date'".	(isset($_filter_act[11]) ? $_filter_act[11] : ''),
/*12*/	12=>"it_code='$_code' AND rjed_type=$_type AND rjed_wh_location=$_loc AND rjed_expired_date=date '$_date'".		(isset($_filter_act[12]) ? $_filter_act[12] : ''),

/*13*/	13=>"a.it_code='$_code' AND ped_type=$_type AND ped_wh_location=$_loc AND ped_expired_date=date '$_date'".		(isset($_filter_act[13]) ? $_filter_act[13] : ''),
/*14*/	14=>"a.it_code='$_code' AND ped_type_x=$_type AND ped_wh_location=$_loc AND ped_expired_date=date '$_date'".	(isset($_filter_act[14]) ? $_filter_act[14] : '')
);
...
SELECT
	...
	false AS qty_value,
	rjed_qty AS qty 
	FROM ".ZKP_SQL."_tb_reject_ed WHERE $strWhere[12] UNION 

SELECT
	ped_submited_timestamp AS timestamp, 
  	to_char(ped_submited_timestamp, 'dd / Mon / yy hh24:mi:ss') AS cfm_timestamp,
	31 AS type_activity,
	'Outgoing (Change Type)' AS name_activity,
	null AS idx_activity,
	por_doc_number AS doc_activity,
	por_doc_date AS date_activity,
	false AS qty_value,
	ped_qty AS qty 
	FROM ".ZKP_SQL."_tb_po_replace JOIN ".ZKP_SQL."_tb_po_replace_ed AS a USING (por_idx) WHERE $strWhere[13]  UNION
SELECT
	ped_submited_timestamp AS timestamp, 
  	to_char(ped_submited_timestamp, 'dd / Mon / yy hh24:mi:ss') AS cfm_timestamp,
	10 AS type_activity,
	'Incoming (Change Type)' AS name_activity,
	null AS idx_activity,
	por_doc_number AS doc_activity,
	por_doc_date AS date_activity,
	true AS qty_value,
	ped_qty AS qty 
	FROM ".ZKP_SQL."_tb_po_replace JOIN ".ZKP_SQL."_tb_po_replace_ed AS a USING (por_idx) WHERE $strWhere[14]
ORDER BY timestamp
";
		...
		<td align="right">
			...
			<select name="cboTypeActivity">
				...
				<optgroup label="-In Stock-" style="background-color:#FFFF99;color:darkblue;">
					...
					<option value="16">Borrow</option>
					<option value="18">Incoming (Move Type)</option>
					<option value="19">Incoming (Move Location)</option>
					<option value="10">Incoming (Change Type)</option>
				</optgroup>
				<optgroup label="-Out Stock-" style="background-color:#FFCC99;color:darkblue">
					...
					<option value="25">Lend</option>
					<option value="29">Delete expired stock</option>
					<option value="28">Outgoing Return (Move Type)</option>
					<option value="30">Outgoing (Move Location)</option>
					<option value="31">Incoming (Change Type)</option>
				</optgroup>
			</select>
			Period :
			<select name="_month">
...
<table width="100%" class="table_c">
	...
	<tr>
		...
		<td width="10%" align="right"><?php echo number_format($column['qty'],2) ?></td>
		<?php } else { ?>
		<td width="10%" align="right" style="color:red"><?php echo number_format($column['qty'],2) ?></td>
		<?php } ?>
	</tr>
	<?php
