...
//GLOBAL
$left_loc 	  = "daily_using_balance_by_reference.php";
...
$_status    = isset($_GET['cboStatus']) ? $_GET['cboStatus'] : "cfm";
...
<table width="100%" class="table_layout">
    <tr>
        <td width="70%" rowspan="2"><h4>[<font color="#446fbe"><?php echo strtoupper($currentDept) ?></font>] REQUEST BALANCE SUMMARY by reference</h4></td>
        <td> CATEGORY </td>
    </tr>
        <td>
...
<table width="100%" class="table_box">
	<tr>
		<td align="right">
			<a href="./daily_using_balance_by_item.php?<?php echo getQueryString()?>">View By Item</a>
		</td>
	</tr>
</table>
<?php require_once APP_DIR . "_include/demo/report/rpt_daily_using_balance_by_reference.php" ?>
<script language="javascript1.2" type="text/javascript">
    ...
    setSelect(f.cboStatus, "<?php echo isset($_GET['cboStatus']) ? $_GET['cboStatus'] : "cfm"?>");
...
?>
	...
	f.cboType.onchange		= f.cboDept.onchange;
	...
	function checkAll(o) {
		...
		for (var i = 0; i < oCheck.length; i++) {
			if (oCheck[i].type == "checkbox" && oCheck[i].name == "chkRequest[]") {
				oCheck[i].checked = o;
			}
		}
	}
	...
	function summarizeRequest() {
		...
		for (var i = 0; i < oCheck.length; i++) {
			if (oCheck[i].type == "checkbox" && oCheck(i).name == "chkRequest[]" && oCheck(i).checked) {
				if(keyword == '') {
		...
		if(keyword == '') {
			alert("You haven't checked any Request.\nPlease check first");
			return;
		}
		...
		var win = window.open(
			'./p_summary_request.php?_code='+ keyword,
			'',
