...
$strwhere 	= array();
$strwhereLoc = ($_location == 0) ? "":"AND stk_wh_location = $_location";

//DEFAULT PROCESS
$sqlQuery = new strSelect(
"SELECT 
  it_code, it_type, it_model_no, it_desc, 
  (SELECT sum(stk_qty) FROM ".ZKP_SQL."_tb_stock_v2 WHERE it_code = it.it_code AND stk_type = 1 $strwhereLoc) AS normal_qty, 
  (SELECT sum(stk_qty) FROM ".ZKP_SQL."_tb_stock_v2 WHERE it_code = it.it_code AND stk_type = 2 $strwhereLoc) AS dtd_qty,
  (SELECT sum(stk_qty) FROM ".ZKP_SQL."_tb_stock_v2 WHERE it_code = it.it_code $strwhereLoc) AS qty
FROM 
  ".ZKP_SQL."_tb_stock_v2  AS it
...
function fillOptionInit() {
	setSelect(window.document.frmSearch._location, <?php echo $_location ?>);
...
<table width="100%" class="table_box">
    ...
    <tr>
    	<td align="right">
            ...
            <input type="checkbox" name="chkNull"<?php  echo ($show_null=='true') ? " checked":"" ?> onclick="searchItem()"> Include 0 qty
		</td>
		<th width="10%">
...
window.document.frmSearch.txtKeyword.onkeypress = function() {
	if(window.event.keyCode == 13) {
		searchItem();
	}
}
window.document.frmSearch.txtRow.onkeypress = window.document.frmSearch.txtKeyword.onkeypress;
...
function searchItem() {
	...
	if(document.frmSearch.chkNull.checked == false) {
		...
		document.frmSearch.show_null.value = true;
	}
...
<table width="100%" class="table_c">
	...
	<tr>
		...
		<th colspan="2" width="12%">STOCK</th>
		<th rowspan="2" width="7%">TOTAL</th>
	</tr>
	<tr>
		<th width="6%">VAT</th>
		<th width="6%">NON VAT</th>
	</tr>
	<?php } else {  ?>
	<tr>
		<th width="4%">No</th>
...
<?php while ($column =& fetchRowAssoc($result)) { ?>
	<tr style="font-color:<?php echo $column['it_color'] ?>">
		...
		<td><a href="<?php echo HTTP_DIR . "$currentDept/$moduleDept/" ?>history_stock.php?_code=<?php echo urlencode(html_entity_decode($column['it_code'], ENT_QUOTES));?>&cboLocation=<?php echo $_location ?>"><div class="bar"><?php echo trim($column['it_model_no'])?></div></a></td>
		<td><?php echo cut_string($column['it_desc'],65);?></td>
		<?php if($currentDept == 'report_all') { ?>
		<td align="right">
			<?php
			if($column['normal_qty'] != 0) {
				echo number_format($column['normal_qty'],1);
			} else echo "0";
			?>	
		</td>
		<td align="right">
			<?php
			if($column['dtd_qty'] != 0) {
				echo number_format($column['dtd_qty'],1);
			} else echo "0";
			?>
		</td>
		<?php } ?>
		<td align="right">
			<?php
			if($column['qty'] != 0) {
				echo number_format($column['qty'],1);
			} else echo "0";
			?>
		</td>
	</tr>
