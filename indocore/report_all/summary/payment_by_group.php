...
?>
<h4>[<font color="#446fbe">GENERAL</font>] PAYMENT SUMMARY by group </h4>
...
?>
		...
		<td>
			<select name="cboFromMonth">
				<option value=""></option>
<?php foreach($month as $value => $key) { 
echo "\t\t\t\t<option value=\"$value\">$key</option>\n";
}?>
			</select>
			<select name="cboFromYear">
				<option value=""></option>
<?php foreach($year as $value => $key) { 
echo "\t\t\t\t<option value=\"$value\">$key</option>\n";
}?>
		</td>
		<td>
...
<iframe id="rightFrame" src="i_payment_by_group.php?<?php echo getQueryString()?>" frameborder="0" width="100%" height="1000px" name="iFrm"></iframe><br /><br />
<script language="javascript1.2" type="text/javascript">
	...
	setSelect(f.cboFromMonth, "<?php echo isset($_GET['cboFromMonth']) ? $_GET['cboFromMonth'] : ""?>");
	setSelect(f.cboFromYear, "<?php echo isset($_GET['cboFromYear']) ? $_GET['cboFromYear'] : ""?>");
	...
	f._cug_code.onchange = function() {
		...
		//if(f.cboFromMonth.value!='' && f.cboFromYear.value=='') 	 { f.cboFromYear.value="<?php echo date("Y") ?>"; }
		//else if(f.cboFromMonth.value=='' && f.cboFromYear.value!='') { f.cboFromMonth.value="<?php echo date("n") ?>"; }
		f.submit();
	}
	...
	f.cboFromMonth.onchange		= f._cug_code.onchange;
	f.cboFromYear.onchange		= f._cug_code.onchange;
	f.cboBank.onchange		= f._cug_code.onchange;
	f.chkCompany.onclick		= f._cug_code.onchange;
