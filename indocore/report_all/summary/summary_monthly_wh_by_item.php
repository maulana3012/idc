...
$_filter_order	= isset($_GET['cboFilterOrder']) ? $_GET['cboFilterOrder'] : "all";
...
$_inc_idc		= isset($_GET['chkIncIdc']) ? $_GET['chkIncIdc'] : "";
		...
		<td>
			<select name="cboFilterDoc" onchange="submitSearch()">
				...
				<optgroup label="-In Stock-" style="font-style:italic;">
					...
					<option value="in-4">Return Trade-In</option>
				</optgroup>
			</select>
		</td>
		<td>
			<select name="cboFilterOrder" onchange="submitSearch()">
				<option value="all">==ALL==</option>
...
echo "\t\t\t\t<option value=\"$value\">$key</option>\n";
		...
		<td><input type="checkbox" name="chkIncMed" onClick="submitSearch()"<?php echo ($_inc_med=='on') ? ' checked' : ''?>></td>
		<td><input type="checkbox" name="chkIncIdc" onClick="submitSearch()"<?php echo ($_inc_idc=='on') ? ' checked' : ''?>></td>
	</tr>
</table>
<div align="right"><span class="comment"><i>*) Based on warehouse data (confirm warehouse date), not sales data</i></span></div>
<br />
</form>
<iframe id="rightFrame" src="i_summary_monthly_wh_by_item.php?<?php echo getQueryString()?>" frameborder="0" width="100%" height="1000px" name="iFrm"></iframe><br /><br />
* <small><i>Checklist MED checkbox to include qty outgoing, incoming from Customer code [0MSD] Medisindo. And so IDC checkbox.</i></small>
