...
//-------------------------------------------------------------------------------------------- DEFAULT PROCESS
$sql	= "SELECT * FROM ".ZKP_SQL."_tb_po_local JOIN ".ZKP_SQL."_tb_pl_local AS pl USING(po_code) WHERE po_code = '$_code' and pl_no = '$_pl_no'";
$result = query($sql);
...
SELECT 
 ...
 it.it_code,		--1
 it.it_ed,			--2
 substr(it_model_no,1,15) AS plit_item,	--3
 substr(it_desc,1,28) AS plit_desc,		--4
 plit.plit_qty,		--5
 (SELECT poit_unit FROM ".ZKP_SQL."_tb_po_local_item AS poit WHERE po_code='$_code' and poit.it_code=plit.it_code limit 1) AS plit_unit,	--6
 plit.plit_qty - ".ZKP_SQL."_arrivedQty(3, po_code|| '-' ||pl_no, it.it_code) AS remain_qty --7
FROM
...
<input type="hidden" name="_sp_code" value="<?php echo $column['sp_code']?>">
<input type="hidden" name="_pl_type" value="<?php echo $column['po_type']?>">
<span class="bar_bl">PL INFORMATION</span>
<table width="100%" class="table_box">
	<tr>
		<th width="15%">PL NO</th>
		<td width="22%"><b><?php echo $column["po_code"] ?>&nbsp; &nbsp;#<?php echo $column["pl_no"] ?></b></td>
		<th width="7%">
...
foreach($group0 as $total1 => $group1) {
	...
	cell_link('<b>'.$rd[$rdIdx][1].'</b>', ' valign="top" align="center" rowspan="'.$rowSpan.'"',
		' href="detail_confirm_pl_local.php?_inlc_idx='.$rd[$rdIdx][0].'"');	//arrival date
