...
if(ckperm(ZKP_DELETE, HTTP_DIR . "$currentDept/$moduleDept/index.php", 'unconfirmed')) {
	...
	if (isZKError($result)) {
		...
		$M->goErrorPage($result, HTTP_DIR . "$currentDept/$moduleDept/detail_return_dt.php?_inc_idx=$_inc_idx&_std_idx=$_std_idx");
	}
...
SELECT
  ...
  b.init_reject_qty,
  CASE
  	WHEN b.init_type=1 THEN 'VAT'
  	WHEN b.init_type=2 THEN 'NON'
  END AS type
...
<table width="100%" class="table_box">
	<tr>
		<td colspan="5"><strong class="info">RETURN INFORMATION</strong></td>
		<td colspan="2" align="right"><span class="comment"><i>Confirm by : <?php echo $column['inc_confirmed_by_account'].date(', d-M-Y g:i:s',strtotime($column['inc_confirmed_timestamp'])) ?></i></span></td>
	</tr>
	<tr>
