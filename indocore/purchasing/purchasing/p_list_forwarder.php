...
$sqlQuery = new strSelect("SELECT fw_code, fw_full_name, fw_phone, fw_fax, fw_mobile_phone, fw_contact_name  FROM ".ZKP_SQL."_tb_forwarder");
$sqlQuery->setWhere("%s ILIKE '%s%%'", array("fw_full_name" => "_name"), "AND");
$sqlQuery->setOrderBy("fw_full_name");
...
while ($rows =& fetchRow($result,0)) {
	printf("sp['%s']=['%s','%s','%s','%s','%s','%s'];\n",
		...
		addslashes($rows[4]),	//mobile phone
		addslashes($rows[5])	//contact
	);
}
?>
...
function fillField(idx) {
	...
	f._fw_code.value	= sp[idx][0];
	f._fw_name.value	= sp[idx][1];
	f._fw_phone.value	= sp[idx][2];
	f._fw_fax.value		= sp[idx][3];
	f._fw_mobile_phone.value	= sp[idx][4];
	f._fw_contact.value	= sp[idx][5];
...
<table width="100%" class="table_box">
	<tr>
		<td rowspan="2" width="40%"><h4>FORWARDER LIST</h4></td>
		...
		<th rowspan="2" width="15%">
			...
			<a href="javascript:document.location.href='p_list_forwarder.php'"><img src="../../_images/icon/list_mini.gif" alt="see all"></a>
		</th>
	</tr>
	<tr>
...
?>
	  <tr>
		...
		<td width="15%"><a href="javascript:fillField('<?php echo addslashes(html_entity_decode($column['fw_code']))?>')"><?php echo $column['fw_code']?></a></td>
		<td><?php echo $column['fw_full_name']?></td>
	  </tr>
	  <?php
	}//end repeat rows
	?>
