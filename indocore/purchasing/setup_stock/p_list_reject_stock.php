...
$stock_sql	= "SELECT 
				...
				FROM ".ZKP_SQL."_tb_item WHERE it_ed is false ORDER BY it_code";
$stock_res	= query($stock_sql);
echo "var stock = new Array();\n";
while ($rows =& fetchRow($stock_res)) {
	printf("stock['%s'] = ['%s','%s','%s',%s,%s,%s,%s];\n",
		trim($rows[0]),			//it_code
		trim($rows[0]),			//it_code
		$rows[1],				//it_model_no
		$rows[2],				//desc
...
function fillItem(idx, type, loc) {
	...
	f._location_to.value 		= loc;
	f._location_to.disabled		= true;
...
function createNewItem() {
	...
	if (f._code.value.length <= 0) {
		...
		f._code.focus();
		return;
	} else if (f._sn.value.length <= 0) {
		alert("Please input serial number");
		f._sn.focus();
		return;
	} else if(f._date.value.length <= 0) {
		alert("Please input expired warranty");
		f._date.focus();
		return;
	} else if (d == null) {
		alert("Please input expired warranty with proper format");
		f._date.value = '';
		f._date.focus();
		return;
	}

	f._date.value	= formatDate(d, "d-NNN-yyyy");
...
<table width="100%" class="table_box">
	<tr>
		<td>
			...
			<small>* Printed for move stock to reject</small>
			</font>
			</strong>
		</td>
		<td align="right"><a href="javascript:window.location.href='p_information.php?_id=stock_to_reject'"><img src="../../_images/icon/info.gif" alt="Help solution ;)"></a></td>
	</tr>
</table><hr>
<table width="100%" class="table_box">
...
<table width="100%" class="table_c">
	...
	<tr>
		...
		<?php if($column['vat_stock'] > 0 && $column['it_ed']=='f') { ?>
		...
		<?php } ?>
		<?php if($column['non_stock'] > 0 && $column['it_ed']=='f') { ?>
...
<table width="100%" class="table_box" cellspacing="1">
	...
	<tr>
		<td>
			<input type="text" name="_code" class="req" style="width:100%" maxlength="6" readonly>
			<input type="hidden" name="_model_no">
		</td>
		<td><input type="text" name="_sn" class="req" style="width:100%" maxlength="32"></td>
		<td><input type="text" name="_date" class="reqd" style="width:100%" maxlength="15" onKeyPress="if(window.event.keyCode==13) {createNewItem()}"></td>
		<td><input type="text" name="_qty" class="fmtn" style="width:100%" readonly value="1"></td>
		<td><input type="text" name="_desc" class="fmt" style="width:100%" maxlength="255" onKeyPress="if(window.event.keyCode==13) {createNewItem()}"></td>
		<td align="center">
