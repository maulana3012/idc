...
$_status	= isset($_GET['cboStatus']) ? $_GET['cboStatus'] : "pending";
...
function insertED(code,item,type) {
	...
	if(window.document.frmReturn.btnReturn.disabled) { return;}

	wInputED = window.open(
		'./p_input_ed.php?_act=move_type&_code='+code+'&_item='+item+'&_type='+type+'&_type='+type+'&_loc='+loc,
		'move_type',
...
function initPage() {
	var f = window.document.frmReturn;
<?php if($_type=='all' || $_loc=='all') { ?>
	f.btnReturn.disabled	= true;
<?php } ?>
}
</script>
</head>
...
if($isShowMessage != '') {
	...
	if($isShowMessage == "true") {
		...
		echo "\t<img src=\"../../_images/icon/check.gif\"> Success save date\n";
		echo "</span><br /><br />\n";
	} else if($isShowMessage == "error") {
		echo "<span style=\"font-family:Courier;color:red\">";
		echo "\t<img src=\"../../_images/icon/alert.gif\"> Error while save data!";
		echo "</span><br /><br />";
	}
	echo "<div align=\"right\">";
	echo "\t<a href=\"list_borrow_by_item.php\" style=\"font-family:Courier;color:#015396\">Close this section <img src=\"../../_images/icon/close_mini.gif\"></a>";
	echo "</div><hr><br /><br />";
}
?>
<strong style="font-size:18px;font-weight:bold">
[<font color="#446fbe"><?php echo strtoupper($currentDept) ?></font>] BORROW SUMMARY by item<br />
</strong>
<small class="comment">* Please select location &amp; type stock to move stock type</small>
<br /><br />
	...
	<tr>
		...
		<td>
			<select name="cboStatus">
				...
				<option value="retuned">Retuned</option>
			</select>
		</td>
		<td>
...
<form name="frmReturn" action="<?php echo $_SERVER['PHP_SELF']?>" method="POST">
<input type="hidden" name="p_mode" value="return_borrow">
<?php require_once APP_DIR . "_include/purchasing/report/setup_stock/rpt_list_borrow_by_item.php" ?>
...
<table width="100%" class="table_layout">
	<tr>
		...
		<td align="right">
			<button name='btnReturn' class='input_btn' style='width:80px;'><img src="../../_images/icon/setting_mini.gif" align="middle"> &nbsp; Return</button>&nbsp;
...
<script language="javascript1.2" type="text/javascript">
	...
	setSelect(f.cboStatus, "<?php echo isset($_GET['cboStatus']) ? $_GET['cboStatus'] : "pending"?>");
	...
	f.cboStatus.onchange	= f.cboLocation.onchange;
	...
	function checkAll(o) {
		var oCheck = window.document.frmReturn.tags("INPUT");
	...
	window.document.frmReturn.btnSummarize.onclick = function() {
		var oCheck		 = window.document.frmReturn.tags("INPUT");
		var keyword		 = '';
		var counter		 = 0;
...
?>
	...
	window.document.frmReturn.btnReturn.onclick = function() {
		var o			 = window.document.frmReturn;
...
//				else if(oCheck(i+4).value=='2' && oCheck(i+5).value=='1') {stock_id=3;type='vat';loc='DNR';}
//				else if(oCheck(i+4).value=='2' && oCheck(i+5).value=='2') {stock_id=4;type='non';loc='DNR';}
				...
				if(parseFloat(stock[oCheck(i+1).value][stock_id]) < parseFloat(oCheck(i+6).value)) {
					...
					if(confirm("Some of the item have not available stock.\nDo you want to replace from available stock?")) {
						var f = window.document.frmSearch;
						window.location = "input_move_type.php?cboLocation=" + f.cboLocation.value +
															 "&cboType=" + f.cboType.value + 
															 "&s_mode=" + f.s_mode.value + 
															 "&some_date=" + f.some_date.value + 
															 "&period_from=" + f.period_from.value + 
															 "&period_to=" + f.period_to.value
					}
					return;
				}
