...
//---------------------------------------------------------------------------------------------- DEFAULT PROCESS
$sql	= "SELECT * FROM ".ZKP_SQL."_tb_initial_indocore_stock AS a JOIN ".ZKP_SQL."_tb_item AS b USING(it_code) WHERE it_code = '$_code' AND ist_type = $_type AND ist_wh_location = $_location";
$result = query($sql);
$column = fetchRowAssoc($result);

$sql_ed = "SELECT * FROM ".ZKP_SQL."_tb_expired_initial WHERE it_code = '$_code' AND eni_type = $_type AND eni_wh_location = $_location ORDER BY eni_expired_date";
$res_ed = query($sql_ed);
?>
<html>
<head>
...
<table width="100%" class="table_l">
	...
	<th>LOCATION</th>
	<td>
		<input type="radio" name="_location" value="1" disabled<?php echo ($column["ist_wh_location"]==1) ? ' checked':''?>>INDOCORE
		<input type="radio" name="_location" value="2" disabled<?php echo ($column["ist_wh_location"]==2) ? ' checked':''?>>DNR
	</td>
</tr>
<tr>
	<th>TYPE</th>
	<td>
		<input type="radio" name="_type_item" value="1" disabled<?php echo ($column["ist_type"]==1) ? ' checked':''?>>VAT &nbsp;
		<input type="radio" name="_type_item" value="2" disabled<?php echo ($column["ist_type"]==2) ? ' checked':''?>>NON
	</td>
</tr>
<tr>
	<th>QTY</th>
	<td><input type="text" name="_init_qty" class="fmtn" style="width:15%" value="<?php echo number_format($column["ist_qty"],2)?>" readonly></td>
</tr>
</table>
</form>
    	</td>
    	...
    	<td>
			...
			<table width="50%" class="table_l">
				...
				<?php while($col = fetchRowAssoc($res_ed)) { ?>
					<tr id="<?php echo date('j-M-Y',strtotime($col["eni_expired_date"])) ?>">
						<td><input type="hidden" name="_ed_date[]" value="<?php echo date('j-M-Y',strtotime($col["eni_expired_date"])) ?>"><?php echo date('M-Y',strtotime($col["eni_expired_date"])) ?></td>
						<td align="right"><input type="hidden" name="_ed_qty[]" value="<?php echo $col["eni_qty"] ?>"><?php echo number_format($col["eni_qty"],2) ?></td>
					</tr>
				<?php } ?>
				</tbody>
			</table>
