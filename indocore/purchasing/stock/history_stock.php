...
SELECT 	
  it_code, it_model_no,
  (SELECT sum(log_qty) FROM ".ZKP_SQL."_tb_stock_logs WHERE it_code='$_code' AND log_wh_location=$_location AND log_type=$_type AND log_document_type BETWEEN 9 AND 20 AND log_qty_status=true ) AS in_stock,
  (SELECT sum(log_qty) FROM ".ZKP_SQL."_tb_stock_logs WHERE it_code='$_code' AND log_wh_location=$_location AND log_type=$_type AND log_document_type BETWEEN 21 AND 32 AND log_qty_status=true ) AS out_stock, 
  stk_qty,
  to_char(stk_updated, 'dd-Mon-yy hh24:mi:ss'),
  ".ZKP_SQL."_getLastBalanceQty('$_code', $_type, $_location, $_filter_by, date '$period_from 00:00:00'),
...
WHERE it_code = '$_code' AND stk_wh_location = $_location AND stk_type = $_type";
...
FROM ".ZKP_SQL."_tb_stock_logs WHERE $strWhere ORDER BY log_cfm_timestamp, log_code";
...
function seeDetail() {
	...
	var type	= window.document.frmSearch.cboType.value;
	var loc		= window.document.frmSearch.cboLocation.value;

	window.location.href = 'history_stock.php?_code=' + code + '&cboType=' + type + '&cboLocation=' + loc;
}
...
function initPage() {
	setSelect(window.document.frmSearch.cboType, "<?php echo $_GET['cboType'] ?>");
...
<fieldset>
	<legend> <img src="../../_images/icon/history.gif" align="middle"> <span style="color:#446fbe;font-family:courier;font-weight:bold">COMPLETE STOCK HISTORY </span></legend>
	<table width="100%" class="table_box">
		<tr>
			<td width="35%"></td>
			<td>Type Activity :</td>
			<td>
				<select name="cboTypeActivity">
					...
					<optgroup label="-In Stock-" style="background-color:#FFFF99;color:darkblue;">
						...
						<option value="9">PO+ (Shadow)</option>
					...
					<optgroup label="-Out Stock-" style="background-color:#FFCC99;color:darkblue">
						...
						<option value="32">PO- (Shadow)</option>
					</optgroup>
				</select>
			</td>
			<td>
		...
		<tr>
			<td colspan="4" align="right"><input type="checkbox" name="chkIncNote" onClick="includeNote()" <?php echo ($_show_note == 'true')?'checked':'' ?>> <i>show cancelled record</i></td>
		</tr>
	</table><br />
	<table width="100%" class="table_n">
		<tr height="40px">
	...
	<?php while($log =& fetchRowAssoc($res_history)) { ?>
		<tr <?php echo ($log["log_qty_status"]=='f')?"style=\"text-decoration:line-through;\"":"" ?>>
			...
			<td  width="20%" align="right">
				<img src="../../_images/icon/comment.gif" alt="Unconfirmed by: <?php echo $log["log_uncfm_by_account"].", ".date('d-M-Y H:i:s',strtotime($log["log_cfm_timestamp"])) ?>">
...
<fieldset>
	...
	<form name="frmEDStock" class="post" action="p_list_ed.php">
...
function findEDLog() {
	...
	wSearchItem = 
		window.open("p_list_ed.php?_code="+code+"&_model="+model+"&_type="+type+"&_loc="+loc+"&_month="+month+"&_year="+year+"&cboTypeActivity="+filter_by,
		'',
...
<script language="javascript1.2" type="text/javascript">
	...
	function goPageLocation() {
		var f = window.document.frmSearch;
		window.location.href = "history_stock.php?_code="+trim(f._code.value)+"&cboType="+f.cboType.value+"&cboLocation="+f.cboLocation.value+"&cboTypeActivity="+f.cboTypeActivity.value+
...
function downloadPDF(file) {
	...
	winforPrint.document.location.href = "<?php echo HTTP_DIR . "$currentDept/$moduleDept/pdf/" ?>download_pdf.php?_file="+file;
}
</script>
          </td>
        </tr>
