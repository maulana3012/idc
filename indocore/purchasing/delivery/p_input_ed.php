...
$_type		= isset($_GET['_type']) ? $_GET['_type'] : 1;
...
function submitSearch() {
	...
	if(f._type_item[0].checked==true && f._location[0].checked==true) {
		loc = 1;type = 1;
	} else if(f._type_item[0].checked==true && f._location[1].checked==true) {
		loc = 2;type = 1;
	} else if(f._type_item[1].checked==true && f._location[0].checked==true) {
		loc = 1;type = 2;
	} else if(f._type_item[1].checked==true && f._location[1].checked==true) {
		loc = 2;type = 2;
	}
...
function createNewED() {
	...
	f._qty.value = removecomma(numFormatval(f._qty.value+'',0));
...
<table width="100%" class="table_l">
	...
	<tr>
		...
		<td width="15%"><input type="radio" name="_location" value="1" id="ind" onclick="submitSearch()" <?php echo ($_loc==1)?'checked':'' ?>><label for="ind">INDOCORE</label></td>
		<td><input type="radio" name="_location" value="2" id="dnr" onclick="submitSearch()" <?php echo ($_loc==2)?'checked':'' ?>><label for="dnr">DNR</label></td>
	</tr>
	<tr>
		<th>TYPE</th>
		<td><input type="radio" name="_type_item" value="1" id="1" onclick="submitSearch()" <?php echo ($_type==1)?'checked':'' ?>><label for="1">VAT</label></td>
		<td><input type="radio" name="_type_item" value="2" id="2" onclick="submitSearch()" <?php echo ($_type==2)?'checked':'' ?>><label for="2">NON</label></td>
	</tr>
</table>
</form>
    	</td>
