...
//========================================================================================= unconfirm request
if (ckperm(ZKP_DELETE, HTTP_DIR . "$currentDept/$moduleDept/detail_return.php?_code=$_code", 'uncfm_request')) {

	$result = executeSP(
		ZKP_SQL."_unCfmReturnDemoByMarketing",
		"$\${$_code}$\$"
	);

	if(isZKError($result)) {
		$M->goErrorPage($result, HTTP_DIR . "$currentDept/$moduleDept/detail_return.php?_code=".urlencode($_code));
	}
	goPage(HTTP_DIR . "$currentDept/$moduleDept/confirm_return.php?_code=".urlencode($_code));
}
...
<?php if($S->getValue("ma_authority") & 1024) { ?>
...
<table width="100%" cellpadding="0">
 	<tr>
		...
		<td><strong>Unconfirm Return Request</strong></td>
		<td align="right">
...
window.document.frmUnConfirm.btnUnCfmWarehouse.onclick = function() {
	if(confirm("Are you sure to unconfirm return demo unit?")) {
