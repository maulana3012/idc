...
if(isset($_GET['lastCategoryNo']) && $_GET['lastCategoryNo'] > 0) {
	...
	$sqlQuery->whereCaluse = "demo_qty != 0 and icat_midx IN (" . (empty($catList[0]) ? "0" : $catList[0]) . ")";
...
function deleteDemo(it_code) {
	var x = (screen.availWidth - 470) / 2;
	var y = (screen.availHeight - 230) / 2;
	var win = window.open(
		'p_delete_demo.php?_it_code='+it_code,
		'',
		'scrollbars,width=470,height=230,screenX='+x+',screenY='+y+',left='+x+',top='+y);
}
</script>
</head>
...
<table width="100%" class="table_box">
	<tr>
		...
		<th width="8%">
			...
			<a href="javascript:document.location.href='list_demo.php'"><img src="../../_images/icon/list_mini.gif" alt="Show all item"></a>&nbsp;
		</th>
	</tr>
</table><br />
...
<table width="100%" class="table_c">
	...
	<tr>
		...
		<?php if($column['it_ed'] == 'f') { ?>
		<td align="right"><a href="javascript:deleteDemo('<?php echo $column['it_code'] ?>')"><?php echo number_format($column['demo_qty'])?></a></td>
		<?php } else { ?>
		<td align="right"><?php echo ($column['it_code']=='2101  ' || $column['it_code']=='2101NE') ? number_format($column['demo_qty'],2) : number_format($column['demo_qty'])?></td>
		<?php } ?>
	</tr>
	<?php } ?>
</table>
