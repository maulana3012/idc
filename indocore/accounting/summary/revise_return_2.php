...
isZKError($res =& query($sql)) ? $M->goErrorPage($res, HTTP_DIR . "$currentDept/$moduleDept/input_return_step_3.php") : false;
$disc = fetchRow($res);
...
<small class="comment">* Issue invoice return only</small>
<hr><br />
...
<?php while($items =& fetchRow($cusitem_res)) { ?>
		<tr id="<?php echo trim($items[0])?>">
			...
			<td><input type="text" class="reqn" name="_cus_it_qty[]" value="<?php echo $items[4]?>" style="width:100%" onBlur="updateAmount()" onKeyUp="formatNumber(this,'dot')"></td>
...
<table class="table_box" width="100%">
	...
	<tr>
		<th>DATE</th>
		<td colspan="2">Tukr Faktur : <input type="text" name="_tukar_faktur_date" class="fmt" size="10" value="<?php echo ($column['turn_tukar_faktur_date'] != '') ? date("j-M-Y", strtotime($column['turn_tukar_faktur_date'])) : ''?>" disabled></td>
	</tr>
</table><br />
