...
//CHECK PARAMETER
$channel = array('A'=>'002', 'D'=>'000', 'H'=>'005', 'P'=>'004');
$_check_code = isset($_REQUEST['_check_code']) ? $_REQUEST['_check_code'] : "";
$_cus_channel	= isset($_REQUEST['_cus_channel'])? $_REQUEST['_cus_channel'] : $_REQUEST['_cus_channel'] = $channel[$department];
...
function fillField(idx) {
	...
	if(target == 'billing') {
		...
		f._cus_code.value		= cus[idx][0];
	} else if(target == 'monthly_sales') {
		f._cus_code.value		= cus[idx][0];
		f._cug_code.value		= '';
		f.submit();
		window.close();
	} else if(target == 'deposit' || target == 'delivery') {
		f._cus_code.value		= cus[idx][0];
