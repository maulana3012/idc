...
print <<<END
	...
	<table width="1735px" class="table_f">
		<tr>
			...
			<th width="420px" colspan="7">CHECK</th>
			<th width="630px" colspan="10">TRANSFER</th>
			<th width="420px" colspan="7">GIRO</th>
...
foreach ($channel as $value => $key) {
	...
	$total[6]  += (isset($amount[$value]['check']['DANAMON3']))		? $amount[$value]['check']['DANAMON3']		: 0;
	$total[7]  += (isset($amount[$value]['check']['BNIS']))			? $amount[$value]['check']['BNIS']		: 0;
	$total[8]  += (isset($amount[$value]['transfer']['BCA1']))		? $amount[$value]['transfer']['BCA1']		: 0;
	$total[9]  += (isset($amount[$value]['transfer']['BCA2']))		? $amount[$value]['transfer']['BCA2']		: 0;
	$total[10] += (isset($amount[$value]['transfer']['MANDIRI']))		? $amount[$value]['transfer']['MANDIRI'] 	: 0;
	$total[11] += (isset($amount[$value]['transfer']['BII1']))		? $amount[$value]['transfer']['BII1']		: 0;
	$total[12] += (isset($amount[$value]['transfer']['BII2']))		? $amount[$value]['transfer']['BII2']		: 0;
	$total[13] += (isset($amount[$value]['transfer']['BII3']))		? $amount[$value]['transfer']['BII3']		: 0;
	$total[14] += (isset($amount[$value]['transfer']['DANAMON']))		? $amount[$value]['transfer']['DANAMON']	: 0;
	$total[15] += (isset($amount[$value]['transfer']['DANAMON2']))		? $amount[$value]['transfer']['DANAMON2']	: 0;
	$total[16] += (isset($amount[$value]['transfer']['DANAMON3']))		? $amount[$value]['transfer']['DANAMON3']	: 0;
	$total[17] += (isset($amount[$value]['transfer']['BNIS']))		? $amount[$value]['transfer']['BNIS']		: 0;
	$total[18] += (isset($amount[$value]['giro']['']))			? $amount[$value]['giro']['']			: 0;
	$total[19] += (isset($amount[$value]['giro']['BII2']))			? $amount[$value]['giro']['BII2']		: 0;
	$total[20] += (isset($amount[$value]['giro']['BII3']))			? $amount[$value]['giro']['BII3']		: 0;
	$total[21] += (isset($amount[$value]['giro']['DANAMON']))		? $amount[$value]['giro']['DANAMON']		: 0;
	$total[22] += (isset($amount[$value]['giro']['DANAMON2']))		? $amount[$value]['giro']['DANAMON2']		: 0;
	$total[23] += (isset($amount[$value]['giro']['DANAMON3']))		? $amount[$value]['giro']['DANAMON3']		: 0;
	$total[24] += (isset($amount[$value]['giro']['BNIS']))			? $amount[$value]['giro']['BNIS']		: 0;
	$total[25] += $t_total;
...
for($i=1; $i<25; $i++) cell(number_format((double)$total[$i]), ' align="right" style="color:brown; background-color:lightyellow"');
cell(number_format((double)$total[25]), ' rowspan="2" align="right" style="color:brown; background-color:lightyellow"');
print "</tr>\n";

print "<tr>\n";
cell(number_format((double)$total[1]+$total[2]+$total[3]+$total[4]+$total[5]+$total[6]+$total[7]), ' colspan="7" align="center" style="color:brown; background-color:lightyellow"');
cell(number_format((double)$total[8]+$total[9]+$total[10]+$total[11]+$total[12]+$total[13]+$total[14]+$total[15]+$total[16]+$total[17]), ' colspan="10" align="center" style="color:brown; background-color:lightyellow"');
cell(number_format((double)$total[18]+$total[19]+$total[20]+$total[21]+$total[22]+$total[23]+$total[24]), ' colspan="7" align="center" style="color:brown; background-color:lightyellow"');
print "</tr>\n";
