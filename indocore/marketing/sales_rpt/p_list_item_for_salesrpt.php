...
function fillItem(idx) {
	...
	f._it_sales_date.focus();
}

//Wrapper function. It call opener's function.
function createNewItem() {
	var f = window.document.frmCreateItem;
	var d = validDate(f._it_sales_date);
	var d_today = parseDate('<?php echo date('Y-m-j')?>', 'prefer_euro_format');

	if (f._it_code.value.length <= 0) {
		alert("Please select the code first");
		f._it_code.focus();
		return;
	} else if (d.getTime() > d_today.getTime()) {
		alert("Sales date is more early than today")
		f._it_sales_date.value = "";
		f._it_sales_date.focus();
		return;
		...
		f._it_qty.focus();
		return;
	} else if (f._it_pay_price.value.length <= 0) {
		alert("Please input apotik price in sales log that you received");
		f._it_pay_price.focus();
		return;
	}

	f._it_qty.value			= removecomma(f._it_qty.value);
	f._it_pay_price.value	= removecomma(f._it_pay_price.value);
	f._it_sales_date.value	= formatDate(d, "d-NNN-yyyy");
...
<table width="100%" class="table_box">
	<tr>
		...
		<th width="15%">
			...
			<a href="javascript:document.location.href='p_list_item_for_salesrpt.php'"><img src="../../_images/icon/list_mini.gif" alt="Show all item"></a>&nbsp;
		</th>
	</tr>
</table><br />
...
<table width="100%" class="table_box">
	<tbody>
		<tr>
			...
			<th width="5%">QTY</th>
			<th width="10%">@PRICE<br/>IN REPORT</th>
			<th width="15%">Fakt/Bill<br/>No.</th>
			<th width="10%">LOP<br/>No.</th>
		</tr>
		<tr>
			...
			<td><input type="text" name="_it_sales_date" style="width:100%" class="reqd"></td>
			<td><input type="text" name="_it_qty" style="width:100%;text-align=right" class="req">
			<td><input type="text" name="_it_pay_price" style="width:100%" class="reqn" onKeyUp="formatNumber(this, 'dot');" onKeyPress="if(window.event.keyCode == 13) createNewItem()"></td>
			<td><input type="text" name="_it_fatur_no" style="width:100%" class="fmt" maxlength="16" onKeyPress="if(window.event.keyCode == 13) createNewItem()"></td>
			<td><input type="text" name="_it_lop_no" style="width:100%" class="fmt" maxlength="10" onKeyPress="if(window.event.keyCode == 13) createNewItem()"></td>
		</tr>
		<tr>
