...
if($_dept == 'A' || $_cug_code != "") {
	$tmp[] = "cug_code = '$_cug_code' AND sl_dept='$_dept'";
	$tmp[] = "sl_date between date '".date("M-d-Y", mktime(0, 0, 0, date('m')-5000, date('d'), date('Y'))). "' and '" . date("M-d-Y") . "'";
	...
	if($_ship_to == '1CBR')  {
		$tmp[] = "cus_code in ('1CBR','1CBB', '1CBC', '1CBP','1CBS', '1CBU', '1CBK', '1CBB')"; 
	} else {
		$tmp[] = "cus_code='$_ship_to'"; 
	}
	$tmp[] = "sl_dept='$_dept'";
	$tmp[] = "sl_date between date '".date("M-d-Y", mktime(0, 0, 0, date('m')-5000, date('d'), date('Y'))). "' and '" . date("M-d-Y") . "'";
	$strWhere = implode(" AND ", $tmp);
