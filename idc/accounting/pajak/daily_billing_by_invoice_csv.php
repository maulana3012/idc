...
SELECT
 ...
 '0' AS var_flag_vat,
  (select cus_type_of_biz from ".ZKP_SQL."_tb_customer where cus_code=b.bill_pajak_to) as var_npwp, 
  bill_pajak_to_name AS var_nama_lawan_transaksi,
...
while ($columns =& fetchRow($result)) {
	for($i=0; $i<19; $i++) {
		...
		else echo html_entity_decode($columns[$i], ENT_QUOTES).';';
	}
	echo "\r";
}
?>