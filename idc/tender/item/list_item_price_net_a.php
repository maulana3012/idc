...
//GLOBAL
$left_loc = "list_item_price_net_a.php";
$_disc_1 = 0.3;
$_disc_2 = 0.325;

//DEFAULT PROCESS
$sqlQuery = new strSelect("SELECT *, ipn_price_kurs*ipn_price_dollar AS ipn_price_rupiah FROM ".ZKP_SQL."_tb_item AS it JOIN ".ZKP_SQL."_tb_item_price_net AS ipn USING (it_code)");
/*echo "<pre>";
var_dump($sqlQuery);
echo "</pre>";*/
$strGet = "";
...
$sqlQuery->setOrderBy("it.it_code, ipn.ipn_idx DESC");
...
while($col =& fetchRowAssoc($result)) {
	$td[] = array(
		...
		$col['ipn_date_from'],		//3
		$col['ipn_date_to'],		//4
		$col['ipn_price_dollar'],	//5
		$col['ipn_price_kurs'],		//6
		$col['ipn_price_rupiah']	//7
	);
...
<table width="100%" class="table_box">
	<tr>
		<td style="color:#000000"><strong class="title">[<font color="#446fbe"><?php echo strtoupper($currentDept) ?></font>] ITEMS PRICE NET HISTORY</strong></td>
...
for ($i = 0; $i < $numItem; $i++) {
	print "<tr>\n";	
	cell_link($td[$rowIdx][0], ' valign="top" rowspan="'.$rowSpan[$i].'"', ' href="./detail_item.php?_code='.$td[$rowIdx][0].'"');
	cell($td[$rowIdx][1], ' valign="top" rowspan="'.$rowSpan[$i].'"');
	cell($td[$rowIdx][2], ' valign="top" rowspan="'.$rowSpan[$i].'"');

	for ($o = 0; $o < $rowSpan[$i]; $o++) {
		if($o > 0) print "<tr>\n";
		cell(date('d-M-Y', strtotime($td[$rowIdx][3])), ' align="center"');
		cell(($td[$rowIdx][4] == '') ? '-' : date('d-M-Y', strtotime($td[$rowIdx][4])), ' align="center"');
		cell(number_format((double)$td[$rowIdx][5],2), ' align="right"');
		cell(number_format((double)$td[$rowIdx][6]), ' align="right"');
		cell(number_format((double)$td[$rowIdx][7]), ' align="right"');
		print "</tr>\n";
		$rowIdx++;
	}
}
