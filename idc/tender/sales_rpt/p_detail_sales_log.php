...
//DELETE
if(ckperm(ZKP_UPDATE, "javascript:window.close();", 'delete')) {
	$_idx = $_POST['_idx'];
...
function initOption() {
	for (i=0; i<mkt.length; i++) {
		addOption(document.frmUpdate._cus_to_responsible_by,mkt[i][1], mkt[i][0]);
	}
	setSelect(window.document.frmUpdate._cus_to_responsible_by, "<?php echo $sl['sl_cus_to_responsible_by'] ?>");
}
</script>
</head>
