...
$sql	= "SELECT *,
	...
	(SELECT book_is_revised FROM ".ZKP_SQL."_tb_booking WHERE book_doc_ref='$_code' AND book_doc_type=3) AS book_is_revised
...
if(numQueryRows($result) <= 0) {
	...
	$message = new ZKError(
		...
		"HAS_NOT_CONFIRMED_BY_WAREHOUSE",
		"DT no $_code has not been confirmed by warehouse. Please check again");
	$M->goErrorPage($message,  HTTP_DIR . "$currentDept/$moduleDept/revise_dt.php?_code=".urlencode($_code));
...
<script language="javascript" type="text/javascript">
	...
	window.document.all.btnUpdate.onclick = function() {
		...
		if(confirm("Are you sure to update?")) {
			if(verify(oForm)){
				oForm.p_mode.value = 'update_dt_revised';
				oForm.submit();
			}
		}
	}
	...
	window.document.all.btnCancel.onclick = function() {
		window.location.href = 'revise_dt.php?_code=<?php echo $_code ?>';
	}
</script>
            <!--END: BODY-->
          </td>
