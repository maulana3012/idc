...
//GLOBAL
$left_loc = "list_item_price_net.php";

//DEFAULT PROCESS
$sqlQuery = new strSelect("SELECT * FROM ".ZKP_SQL."_tb_item AS it JOIN ".ZKP_SQL."_tb_item_price_net AS ipn USING (it_code)");
...
for ($i = 0; $i < $numItem; $i++) {
	...
	for ($o = 0; $o < $rowSpan[$i]; $o++) {
		...
		cell(($td[$rowIdx][5] == '0')?'':number_format((double)$td[$rowIdx][5],2), ' align="right"');
		cell(($td[$rowIdx][6] == '0')?'':number_format((double)$td[$rowIdx][6]), ' align="right"');
		print "</tr>\n";
		$rowIdx++;
	}
}
