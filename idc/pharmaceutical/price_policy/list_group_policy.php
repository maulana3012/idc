...
if ($s_mode == 'date') {
	...
	$period_to = "";
	$sql = "
SELECT ag.ag_idx, ag.cug_code, cug.cug_name, ag.ag_desc,
to_char(ag.ag_date_from, 'dd-Mon-yyyy') AS date_from, to_char(ag.ag_date_to, 'dd-Mon-yyyy') AS date_to,
ag.ag_basic_disc_pct, ag.ag_disc_pct, ag_is_valid, ag_is_apply_all
FROM ".ZKP_SQL."_tb_group_policy AS ag INNER JOIN ".ZKP_SQL."_tb_customer_group AS cug ON (ag.cug_code = cug.cug_code)
WHERE (ag_date_from, ag_date_to + 1) OVERLAPS (DATE '$some_date', DATE '$some_date')
ORDER BY ag.cug_code, ag_idx desc";
...
ag.ag_basic_disc_pct, ag.ag_disc_pct, ag_is_valid, ag_is_apply_all
FROM ".ZKP_SQL."_tb_group_policy AS ag INNER JOIN ".ZKP_SQL."_tb_customer_group AS cug ON (ag.cug_code = cug.cug_code)
WHERE (ag_date_from, ag_date_to + 1) OVERLAPS (DATE '$period_from', DATE '$period_to')
ORDER BY ag.cug_code, ag_idx desc";
}
?>
...
<!-- END: SEARCH -->
<?php require_once APP_DIR . "_include/order/report/rpt_price_policy_group_additional_disc.php" ?>
            <!--END: BODY-->
          </td>
        </tr>
      </table>
