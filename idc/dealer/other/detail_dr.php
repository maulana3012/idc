...
//dr
$sql	= "SELECT *,
".ZKP_SQL."_isValidShowInvoice('".ZKP_FUNCTION."', '$_code','dr') AS doc_accessible,
(SELECT book_idx FROM ".ZKP_SQL."_tb_booking WHERE book_doc_ref='$_code' AND book_doc_type=5) AS book_idx FROM ".ZKP_SQL."_tb_dr WHERE $strWhere";
$result =& query($sql);
$column =& fetchRowAssoc($result);

if($column['doc_accessible']=='f' && $page_permission) {
	$result = new ZKError(
				"NOT_ACCESSIBLE_DOCUMENT",
				"NOT_ACCESSIBLE_DOCUMENT",
				"DR No. <b>$_code</b> is not accessible. Please contact the manager to see the detail.");
	$M->goErrorPage($result,  HTTP_DIR . "$currentDept/summary/daily_billing_by_group.php?cboFilterDoc=DR");
} else if(numQueryRows($result) <= 0) {
	goPage(HTTP_DIR . "$currentDept/$moduleDept/index.php");
} else if($column['dr_cfm_wh_delivery_by_account'] == '') {
	goPage(HTTP_DIR . "$currentDept/$moduleDept/revise_dr.php?_code=".urlencode($column['dr_code']));
}
...
<table width="100%">
  <tr>
	<td valign="top">
		...
		[<font color="#446fbe"><?php echo strtoupper($currentDept) ?></font>] DETAIL DO REPLACE<br />
		</strong>
	</td>
...
<table width="100%" class="table_box">
	...
	<tr>
		...
		<td><?php echo date('j-M-Y', strtotime($column['dr_date'])) ?></td>
	</tr>
	<tr>
		<th>ISSUED BY</th>
		<td><?php echo $column['dr_received_by'] ?></td>
		<th>ISSUED DATE</th>
		<td><?php echo ($column['dr_issued_date']=='')?'':date('j-M-Y', strtotime($column['dr_issued_date'])) ?></td>
	</tr>
	<tr>
		<th>REQUEST BY</th>
		<td><?php echo $column['dr_issued_by'] ?></td>
		<th>TYPE ITEM</th>
		<td>
...
<table width="100%" class="table_box" cellspacing="1">
	<tr>
		...
		<th width="10%">CODE</th>
		<td width="20%"><?php echo $column['dr_cus_to'] ?></td>
		<th width="12%">NAME</th>
		<td colspan="2"><?php echo $column['dr_cus_name'] ?></td>
	</tr>
	<tr>
		<th>ADDRESS</th>
		<td colspan="4"><?php echo $column['dr_cus_address'] ?></td>
	</tr>
	<tr>
		<th>SHIP TO</th>
		<th>CODE</th>
		<td><?php echo $column['dr_ship_to'] ?></td>
		<th>NAME</th>
		<td colspan="2"><?php echo $column['dr_ship_name'] ?></td>
	</tr>
	<tr>
		<th>RETURN REF.</th>
		<th>CODE</th>
...
<strong>[<font color="#446FBE">WAREHOUSE</font>] ITEM LIST</strong>  &nbsp; <i>( <a href="revise_dr_2.php?_code=<?php echo $_code ?>">Revised item</a> )</i>
...
<table class="table_box" width="100%">
	<tr>
		...
		<td>1.<input type="text" name="_delivery_warehouse" class="fmt" size="2" maxlength="2" value="<?php echo $column['dr_delivery_warehouse'] ?>" disabled>ex W/house(P/C/D)</td>
		<td>2.<input type="text" name="_delivery_franco" class="fmt" size="2" maxlength="2" value="<?php echo $column['dr_delivery_franco'] ?>" disabled>Franco(P/D)</td>
		<td>by <input type="text" name="_delivery_by" size="6" class="fmt" value="<?php echo $column['dr_delivery_by'] ?>" disabled></td>
		<td>Freight charge : Rp <?php echo number_format((double)$column['dr_delivery_freight_charge']) ?></td>
	</tr>
	<tr>
		<th>REMARK</th>
		<td colspan="4"><textarea name="_remark" style="width:100%" rows="4" readonly><?php echo $column['dr_remark'] ?></textarea></td>
	</tr>
</table>
<table width="100%" cellpadding="2" cellspacing="0">
	<tr>
		<td><span class="comment"><i>Confirm outgoing item from warehouse by : <?php echo $column['dr_cfm_wh_delivery_by_account'].date(', j-M-Y g:i:s', strtotime($column['dr_cfm_wh_delivery_timestamp']))?></i></span></td>
		<td align="right">
			Rev No:
			<select name="_revision_time" id="_revision_time">
			<?php
				for($counter = $column['dr_revesion_time']; $counter >= 0; $counter--) {
...
<script language="javascript" type="text/javascript">
	...
	window.document.all.btnPrint.onclick = function() {
		...
		winforPrint.document.location.href = "../../_include/other/pdf/download_pdf.php?_code=<?php echo trim($_code)."&_dept=".$currentDept."&_date=".date("Ym", strtotime($column['dr_date']))?>&_rev=" + window._revision_time.value;
	}

	window.document.all.btnList.onclick = function() {
		window.location.href = '<?php echo HTTP_DIR . "$currentDept/$moduleDept/daily_summary_by_group.php?cboSource=dr" ?>';
	}
</script>
