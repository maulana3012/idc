...
$variable = array(
	...
	'dept'		=> array('all'=>'==ALL==', 'A'=>'APOTIK', 'D'=>'DEALER', 'H'=>'HOSPITAL', 'P'=>'PHARMACEUTICAL', 'T'=>'TENDER', 'S'=>'Customer Service'),
	'vat'		=> array('all'=>'==ALL==', 'vat'=>'VAT', 'vat-IO'=>'VAT - IO', 'vat-IP'=>'VAT - IP', 'non'=>'NON VAT'),
);

$mkt_sql = "SELECT ma_idx, ma_account, ma_display_as FROM tb_mbracc WHERE ma_display_as > 0 ORDER BY ma_account";
...
for($i == 0; $i < count($icat_child); $i++) {
	if (count($icat_child[$i]) == 1 && $icat_child[$i][0] == "") {
		$icat_child[$i][0] = "0";
	}
}
...
$sql = "SELECT $bill_qty $turn_qty $dr_qty $bill_amount ". substr($turn_amount,0,-1);
$result =& query($sql);
