...
exit;
select sg_code	from ".ZKP_SQL."_tb_service_reg where substr(sg_code,6,2)::int between 53 and 78;
update ".ZKP_SQL."_tb_service_reg set sg_revesion_time=-1 where substr(sg_code,6,2)::int between 53 and 78;
*/
	$result = executeSP(
		ZKP_SQL."_insertServiceReg",
		"$\${$_reg_date}$\$",
		$_source_customer,
		"$\${$_cus_to}$\$",
		"$\${$_cus_name}$\$",
...
<table width="100%" class="table_box" cellpadding="1" cellspacing="1">
	<tr>
		...
		<td><input type="text" name="_signature_by" class="fmt" size="15" maxlength="32" value="<?php echo ucfirst($S->getValue("ma_account")) ?>"></td>
	</tr>
	<tr>
		<th>REMARK</th>
