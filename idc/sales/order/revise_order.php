...
function initPage(){
	if(window.document.frmInsert.is_ord_lock.value == 't') {
		...
		window.document.frmInsert._po_date.readOnly= true;
...
if($column['ord_type_invoice'] == 0) {
	if($column['ord_cfm_wh_delivery_timestamp'] == '') {
		require_once APP_DIR . "_include/order/tpl_revise_item_1.php"; 
	} else {
		require_once APP_DIR . "_include/order/tpl_revise_item_3.php"; 
	}
} else if($column['ord_type_invoice'] == 1) {
	if($column['ord_cfm_deli_timestamp'] == '') {
		require_once APP_DIR . "_include/order/tpl_revise_item_2.php"; 
	} else {
		require_once APP_DIR . "_include/order/tpl_revise_item_4.php"; 
	}
}
...
<?php
if($column['ord_type_invoice'] == 0 && $column['ord_cfm_wh_delivery_timestamp'] != '') {
	require_once APP_DIR . "_include/order/tpl_detail_delivery.php";
} else if($column['ord_type_invoice'] == 1) {
	require_once APP_DIR . "_include/order/tpl_detail_delivery.php";
}

if ($column['order_used'] == "t") {
	require_once APP_DIR . "_include/order/tpl_detail_return.php";
}

require_once APP_DIR . "_include/order/tpl_detail_attachment.php";
