...
SELECT
  *, ".ZKP_SQL."_isBillingUsed(bill_code) AS billing_used,
  ...
  (SELECT book_is_revised FROM ".ZKP_SQL."_tb_booking WHERE book_doc_ref='$_code' AND book_doc_type=1) AS book_is_revised
...
if(numQueryRows($result) <= 0) {
	goPage(HTTP_DIR . "$currentDept/$moduleDept/index.php");
} else if($column['bill_cfm_wh_delivery_timestamp'] == '') {
	// Harus DO yang Sudah di confirm gudang
	$message = new ZKError(
		"HAS_NOT_CONFIRMED_BY_WAREHOUSE",
		"HAS_NOT_CONFIRMED_BY_WAREHOUSE",
		"Billing no $_code has not been confirmed by warehouse. Please check again");
	$M->goErrorPage($message,  HTTP_DIR . "$currentDept/$moduleDept/revise_billing.php?_code=".urlencode($_code));
...
<script src="../../_script/js_sales.php?_dept=<?php echo $column["bill_dept"]?>&_ship_to=<?php echo trim($column["bill_ship_to"])?>&_cug_code=<?php echo empty($disc[1]) ? "0":$disc[1] ?>" type="text/javascript"></script>
...
[<font color="#446fbe"><?php echo strtoupper($currentDept) ?></font>] DETAIL BILLING ( Revised Item )<br />
</strong>
<small class="comment">* <?php echo $title[$template] ?></small><hr><br />
...
<input type="hidden" name="_type_template" value="1">
...
require_once APP_DIR . "_include/billing/tpl_detail_item_1.php"; 
...
<table width="100%" cellpadding="2" cellspacing="0">
	<tr>
		<td>
			<button name='btnDelete' class='input_red' style='width:120px;' disabled><img src="../../_images/icon/trash.gif" width="15px" align="middle" alt="Delete billing"> &nbsp; Delete billing</button>
		</td>
		<td align="right">
			Rev No:
...
<script language="javascript" type="text/javascript">
	...
	window.document.all.btnUpdate.onclick = function() {
		...
		if(confirm("Are you sure to update?")) {
			if(verify(oForm)){
				oForm.p_mode.value = 'update_billing_revised';
				oForm.submit();
			}
		}
	}
