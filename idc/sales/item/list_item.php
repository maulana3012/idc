...
//DEFAULT PROCESS
$sqlQuery = new strSelect("SELECT *, ".ZKP_SQL."_getUserPrice(it_code, CURRENT_DATE, 'basic',0) AS user_price FROM ".ZKP_SQL."_tb_item");
...
function calcDisc(o) {
	...
	if(window.event.keyCode == 13) {
		...
		var oDiv = window.document.all.tags('div');

		if(obj.name == "_disc_1") {
			for (var i = 13; i < oDiv.length; i = i + 3) {
				oDiv.item(i).innerText =
					numFormatval(
						Math.round(
							(parseInt(removecomma(oDiv.item(i-1).innerText)) - parseInt(removecomma(oDiv.item(i-1).innerText))*disc)/1.1)
					+ '');
			}
		} else {
			for (var i = 14; i < oDiv.length; i = i + 3) {
				oDiv.item(i).innerText =
					numFormatval(
						Math.round(
							(parseInt(removecomma(oDiv.item(i-2).innerText)) - parseInt(removecomma(oDiv.item(i-2).innerText))*disc)/1.1)
					+ '');
			}
		}
	}
...
<table width="100%" class="table_box">
	<tr>
		<td style="color:#000000"><strong class="title">[<font color="#446fbe"><?php echo strtoupper($currentDept) ?></font>] ITEMS LIST</strong></td>
...
<table width="100%" class="table_box">
	<tr height="35px">
		...
		<th width="8%">
			...
			<a href="javascript:document.location.href='list_item.php'"><img src="../../_images/icon/list_mini.gif" alt="Show all item"></a>&nbsp;
		</th>
	</tr>
</table><br />
...
<table width="100%" class="table_c">
	...
	<tr>
		...
		<td><span class="bar"><a href="<?php echo HTTP_DIR . "$currentDept/$moduleDept/" ?>detail_item.php?_code=<?php echo urlencode(html_entity_decode($column['it_code'], ENT_QUOTES));?>"><?php echo $column['it_code']?></a></span></td>
...
<table width="100%" class="table_c">
	<tr>
		...
		<th width="12%">ITEM NO</th>
		<th>DESCRIPTION</th>
		<th width="12%">USER PRICE<br />(Rp.)</th>
		<th width="12%"><input name="_disc_1" size="1" value="<?php echo $_disc_1*100?>" class="fmtn" onKeypress="calcDisc(this)">%</th>
		<th width="12%"><input name="_disc_2" size="1" value="<?php echo $_disc_2*100?>" class="fmtn" onKeypress="calcDisc(this)">%</th>
	</tr>
	<?php while ($column =& fetchRowAssoc($result)) { ?>
	<tr>
		<td><?php echo ++$oPage->serial ;?></td>
		...
		<td><?php echo $column['it_type']?></td>
		<td><?php echo substr($column['it_model_no'], 0, 15)?></td>
		<td><?php echo cut_string($column['it_desc'],36);?></td>
		<td align="right"><div class="unit_price"><?php echo number_format((double)$column['user_price'])?></div></td>
		<td align="right"><div class="disc1"><?php echo number_format(($column['user_price']-$column['user_price']*$_disc_1)/1.1)?></div></td>
		<td align="right"><div class="disc2"><?php echo number_format(($column['user_price']-$column['user_price']*$_disc_2)/1.1)?></div></td>
	</tr>
	<?php } ?>
</table>
<?php } ?>
