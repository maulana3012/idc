...
SELECT
  ...
  b.it_type,
  b.it_desc,
  ".ZKP_SQL."_getUserPrice(b.it_code, CURRENT_DATE) AS user_price,
  CASE
	WHEN ".ZKP_SQL."_getUserPrice(b.it_code, CURRENT_DATE, 'net') IS NOT NULL THEN ".ZKP_SQL."_getUserPrice(b.it_code, CURRENT_DATE, 'net')
	ELSE 0
...
function copyRow() {
	...
	if(f.chkSameValue.checked == true) {
		...
		//if(trim(window.opener.frmInsert._cus_to.value) == '6IDC')
			//f._cus_it_price.value = addcomma(it[f._wh_it_code.value][6]);
		//else
			if(f._dept.value=='A') 
				f._cus_it_price.value = addcomma(it[f._wh_it_code.value][5]);
...
function fillItem(idx) {
	...
	if(f.chkIsPrintCus.checked == true) {
		...
		if(f._dept.value=='A') {
			if(trim(window.opener.frmInsert._cus_to.value) == '6IDC')
				 f._cus_it_price.value = addcomma(it[idx][6]);
			else f._cus_it_price.value = addcomma(it[idx][5]);
		}
...
*/
		oRow.cells(4).innerText = 'Rp. ' + numFormatval(parseFloat(it[itm][5])+'');
	}
...
function initLoad() {
	...
	fillQtyItem();
/*	if(trim(window.opener.frmInsert._cus_to.value) == '6IDC') {
		 window.document.frmCreateItem._cus_it_price.readOnly = true;
		 window.document.frmCreateItem._cus_it_remark.focus();
	} else window.document.frmCreateItem._cus_it_price.focus();*/
}
</script>
</head>
...
<table width="100%" class="table_box">
	...
	<tr>
		<td>
			<button name='btnBack' class='input_sky' style='width:30px;height:25px' onClick="window.location.href='p_list_item_1.php'"><img src="../../_images/icon/back.gif" align="middle" alt="Back"></button>
		</td>
