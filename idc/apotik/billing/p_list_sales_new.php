...
function addSalesList() {
	...
	var oCheck = document.querySelectorAll('#frmList input[type=checkbox]')
	var oRows = document.querySelectorAll('#rowPosition tr');
	...
	for (var i=0; i<count; i++) {
		if(oCheck[j].checked) {
			...
			for (k=0; k<arrInput.length; k++) {
				var oRow	= oRows[arrRow[k]-1];
				var oCell = oRow.querySelectorAll('td');
				document.getElementById('salesList').innerHTML+=createInput('sl_date[]', oCell[1].innerText);
				document.getElementById('salesList').innerHTML+=createInput('cus_code[]', oCell[2].innerText);
				document.getElementById('salesList').innerHTML+=createInput('cus_full_name[]', oCell[3].innerText);
				document.getElementById('salesList').innerHTML+=createInput('faktur_no[]', oCell[4].innerText);
				document.getElementById('salesList').innerHTML+=createInput('lop_no[]', oCell[5].innerText);
				document.getElementById('salesList').innerHTML+=createInput('amount[]', removecomma(oCell[6].innerText));
...
function updateAmount() {
	...
	for (var i=0; i<count; i++) {
		...
		var oCell = oRow.querySelectorAll('td');
		if(oCheck[k].checked) {
			amount	= amount + parseFloat(removecomma(oCell[6].innerText));
		}
		...
		if(oCheck[k].checked) {
			...
			for(var j=0; j<7; j++) { oCell[j].style.backgroundColor = 'lightyellow'; }
		} else {
			for(var j=0; j<7; j++) { oCell[j].style.backgroundColor = 'white'; }
		}
		k++;
	}
