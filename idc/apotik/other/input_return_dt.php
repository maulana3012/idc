...
$sql	= "
SELECT *,
  (SELECT out_idx FROM ".ZKP_SQL."_tb_outgoing WHERE out_doc_ref='$_code') AS out_idx,
  (SELECT out_idx FROM ".ZKP_SQL."_tb_outgoing_v2 WHERE out_doc_ref='$_code') AS out_idx2
FROM ".ZKP_SQL."_tb_dt
WHERE dt_code = '$_code'";
$result =& query($sql);
...
if($column['out_idx2'] == '') {
	$whitem_sql = "
	SELECT
	  it_code, it_model_no, it_desc, otst_type, otst_qty AS dt_qty,
	  ".ZKP_SQL."_getRDTQty('$_code', it_code, otst_type) AS return_qty
	FROM
	  ".ZKP_SQL."_tb_outgoing_stock AS b
	  JOIN ".ZKP_SQL."_tb_item AS a USING(it_code)
	WHERE out_idx = {$column['out_idx']}
	ORDER BY it_code";
} else if($column['out_idx'] == '') {
	$whitem_sql = "
	SELECT
	  it_code, it_model_no, it_desc, otst_type, otst_qty AS dt_qty,
	  ".ZKP_SQL."_getRDTQty('$_code', it_code, otst_type) AS return_qty
	FROM
	  ".ZKP_SQL."_tb_outgoing_stock_v2 AS b
	  JOIN ".ZKP_SQL."_tb_item AS a USING(it_code)
	WHERE out_idx = {$column['out_idx2']}
	ORDER BY it_code";
}
$whitem_res	=& query($whitem_sql);
/*
echo "<pre>";
var_dump($column['out_idx'] == '', $whitem_sql);
echo "</pre>";
exit;
*/
//[CUSTOMER] dt item
