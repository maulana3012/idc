...
if($page_permission) {
	...
	$M->goErrorPage($result, HTTP_DIR . "$currentDept/$moduleDept/detail_confirm_claim.php?_code=$_code&_incl_idx=$_incl_idx");
}

$sql	= "SELECT * FROM ".ZKP_SQL."_tb_claim AS cl JOIN ".ZKP_SQL."_tb_in_claim_v2 AS incl USING(cl_idx) WHERE cl.cl_idx = $_code AND incl.incl_idx = $_incl_idx";
$result = query($sql);
...
SELECT
 ...
 it.it_ed,
 init.init_qty,
 ".ZKP_SQL."_canUsedQty(2,init.cl_idx::varchar, init.incl_idx, it.it_code) AS max_qty,
 ".ZKP_SQL."_checkED(init.incl_idx, it.it_code),
 ".ZKP_SQL."_checkRecapIdx(init.incl_idx, it.it_code)
FROM
 ".ZKP_SQL."_tb_item AS it
 JOIN ".ZKP_SQL."_tb_in_claim_item_v2 AS init USING(it_code)
WHERE init.incl_idx = $_incl_idx AND init.init_qty > 0
ORDER BY it.it_code";
...
SELECT
 ...
 ined_qty
FROM ".ZKP_SQL."_tb_in_claim_item_ed join ".ZKP_SQL."_tb_item using(it_code) 
WHERE incl_idx = $_incl_idx
...
<input type="hidden" name="_cl_idx" value="<?php echo $column['cl_idx']?>">
<input type="hidden" name="_incl_idx" value="<?php echo $column['incl_idx']?>">
<input type="hidden" name="_inpl_type" value="<?php echo $column['incl_type']?>">
<input type="hidden" name="_wh_location" value="<?php echo $column['incl_warehouse']?>">
<input type="hidden" name="_invoice_no" value="<?php echo $column['cl_inv_no']?>">
<input type="hidden" name="_invoice_date" value="<?php echo $column['cl_inv_date']?>">
...
<table width="100%" class="table_box">
	...
	<tr>
		<th>REMARK</th>
		<td colspan="3"><textarea name="_remark" style="width:100%" rows="3"><?php echo $column["incl_remark"] ?></textarea></td>
	</tr>
</table>
<?php
...
FROM
  ".ZKP_SQL."_tb_in_claim_v2
  JOIN ".ZKP_SQL."_tb_in_claim_item_v2 USING(incl_idx)
WHERE incl_idx = $_incl_idx
ORDER BY it_code";
...
<script language="javascript" type="text/javascript">
	...
	window.document.all.btnDelete.onclick = function() {
		if(confirm("Are you sure to delete?")) {
			oForm.p_mode.value = 'delete_claim';
			oForm.submit();
		}
	}
	...
	window.document.all.btnUpdate.onclick = function() {
		...
		if(verify(oForm)){
			if(confirm("Are you sure to change incoming PL?")) {
				oForm.p_mode.value = 'update_claim';
				oForm.submit();
			}
		}
	}
