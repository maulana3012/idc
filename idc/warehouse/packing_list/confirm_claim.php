...
//-------------------------------------------------------------------------------------------- DEFAULT PROCESS
$col[0] = getPLConfirm('Claim', $_code, '', 'info'); 
$col[1] = getPLConfirm('Claim', $_code, '', 'item'); 
$col[2] = getPLConfirm('Claim', $_code, '', 'in_item');
...
while($col_in =& fetchRowAssoc($col[2])) {
	...
	$rd[] = array(
		...
		$col_in['incl_inv_no'],	//2
		$col_in['it_code'], 	//3
		$col_in['it_model_no'],	//4
		$col_in['it_desc'],	//5
		$col_in['init_qty'], 	//6
		$col_in['incl_has_ed'], //7
		$col_in['go_page'] 	//8
	);

	//1st grouping
	if($cache[0] != $col_in['incl_idx']) {
		$cache[0] = $col_in['incl_idx'];
		$group0[$col_in['incl_idx']] = array();
	}
	...
	$group0[$col_in['incl_idx']][$col_in['it_code']] = 1;
}
$g_total = 0;
?>
<html>
...
<input type="hidden" name="_code" value="<?php echo $col[0]['cl_idx']?>">
<input type="hidden" name="_po_code" value="<?php echo '' ?>">
<input type="hidden" name="_sp_code" value="<?php echo $col[0]['cl_sp_code']?>">
<input type="hidden" name="_pl_type" value="<?php echo $col[0]['cl_type']?>">
<input type="hidden" name="_invoice_no" value="<?php echo $col[0]['cl_inv_no']?>">
<input type="hidden" name="_shipment_mode" value="<?php echo $col[0]['cl_shipment_mode']?>">
	<table width="100%" class="table_box">
		...
		<tr>
			...
			<td width="40%"><span class="bar"><?php echo $col[0]["cl_inv_no"] ?></span></td>
		...
		<tr>
			<th>SUPPLIER NAME</th>
			<td><?php echo $col[0]['cl_sp_name']?></td>
			<th>SHIPMENT MODE</th>
			<td>
...
?>
		...
		<tr>
			<th>REMARK</th>
			<td colspan="3"><textarea name="_remark" style="width:100%" rows="3"><?php echo $col[0]["cl_remark"] ?></textarea></td>
		</tr>
	</table><br />
</form>
...
foreach($group0 as $total1 => $group1) {
	...
	foreach($group1 as $total2) {
		...
		cell($rd[$rdIdx][7]."[". trim($rd[$rdIdx][3]) ."] ".$rd[$rdIdx][4]);	//model name
