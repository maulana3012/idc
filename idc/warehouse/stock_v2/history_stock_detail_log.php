...
$sql_last_stock_qty = "
SELECT sum(log_qty) AS qty FROM idc_tb_log_detail WHERE {$strWhere['in'][0]} UNION
SELECT sum(log_qty) AS qty FROM idc_tb_log_detail WHERE {$strWhere['out'][0]}
";
...
echo "<pre>";
#var_dump($sql_history,$strWhere['out'][0]);
echo "</pre>";
...
<?php }} ?>
  ...
  <tr>
    <td colspan="7"><i>Re-initial stock from 22-Jun-2012</i> , see previous history log <a href="history_stock_v1.php?_code=<?php echo $_code ?>&cboType=<?php echo ($_type=='all')?1:$_type ?>&cboLocation=<?php echo $_location ?>">here</a></td>
  </tr>
</table><br /><br />
