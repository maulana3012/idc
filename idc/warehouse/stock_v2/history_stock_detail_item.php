...
SELECT
  ...
  (SELECT sum(stk_qty) FROM idc_tb_stock_v2 WHERE {$strWhere[0][0]}) AS qty,
  (SELECT sum(log_qty) FROM idc_tb_log_detail WHERE {$strWhere[0][1]} AND log_document_type in ($v_log_in)) AS in_qty,
  (SELECT sum(log_qty)*-1 FROM idc_tb_log_detail WHERE {$strWhere[0][1]} AND log_document_type in ($v_log_out)) AS out_qty
FROM
...
<table class="tb_highlight" style="width:100%">
  ...
  <tbody>
	...
	<td><?php echo $v_type[$_type] ?> <a href="javascript:changeItem('type', '<?php echo $_type ?>')"> <?php if($currentDept == 'purchasing') { ?><img src="../../_images/icon/arrow_right.gif" alt="Change type stock"></a><?php } ?></td>
	<td width="25%" class="right"><?php echo $v_location[$_location] ?></td>
    </tr>
  </tbody>
</table>
<table class="tb_highlight" style="width:100%">
