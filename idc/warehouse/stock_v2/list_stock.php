...
"SELECT 
  ...
  (SELECT sum(stk_qty) FROM idc_tb_stock_v2 WHERE it_code = it.it_code AND stk_type = 1) AS normal_qty, 
  (SELECT sum(stk_qty) FROM idc_tb_stock_v2 WHERE it_code = it.it_code AND stk_type = 2) AS dtd_qty,
  (SELECT sum(stk_qty) FROM idc_tb_stock_v2 WHERE it_code = it.it_code) AS qty
FROM 
...
<?php while ($column =& fetchRowAssoc($result)) { ?>
	<tr style="font-color:<?php echo $column['it_color'] ?>">
		...
		<td><a href="<?php echo HTTP_DIR . "$currentDept/$moduleDept/" ?>history_stock.php?_code=<?php echo urlencode(html_entity_decode($column['it_code'], ENT_QUOTES));?>&cboLocation=<?php echo $_location ?>"><div class="bar"><?php echo substr(trim($column['it_model_no']), 0, 25)?></div></a></td>
		...
		<?php } ?>
		<td align="right"><?php echo number_format($column['qty'],1)?></td>
	</tr>
<?php } ?>
</table>
