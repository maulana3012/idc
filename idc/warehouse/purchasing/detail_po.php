...
"SELECT
  ...
  c.it_desc,
  b.init_qty,
  '../packing_list/detail_confirm_pl_local.php?_ver=v1&_inlc_idx='||inlc_idx AS go_page
FROM
...
SELECT
  ...
  c.it_desc,
  b.init_qty,
  '../packing_list/detail_confirm_pl_local.php?_ver=v2&_inlc_idx='||inlc_idx AS go_page
FROM
  ".ZKP_SQL."_tb_in_local_v2 AS a
  JOIN ".ZKP_SQL."_tb_in_local_item_v2 AS b USING(inlc_idx)
  JOIN ".ZKP_SQL."_tb_item AS c USING(it_code)
WHERE po_code = '$_code'
ORDER BY inlc_idx, inlc_checked_date, it_code";
echo "<pre>";
//var_dump($sql_pl);
echo "</pre>";
$rd		= array();
...
foreach($group0 as $total1 => $group1) {
	...
	cell_link("<span class=\"bar\">".$rd[$rdIdx][2]."</span>", ' valign="top" align="center" rowspan="'.$rowSpan.'"',
		' href="'.$rd[$rdIdx][7].'" target="_parent"');	//invoice no
...
<table width="100%" class="table_box">
	<tr>
		...
		<td>
			...
			<ul>
				<li> This PO already already has PL. Delete PL first if you want to revise PO detail</li>
			</ul>
		</td>
	</tr>
</table>
