...
$sql_item = array();
if($column['book_doc_type'] == 1) {
	$sql_item[0] = "SELECT it_code, i.it_model_no, i.it_desc, biit_qty, biit_remark FROM ".ZKP_SQL."_tb_billing JOIN ".ZKP_SQL."_tb_billing_item USING (bill_code) JOIN ".ZKP_SQL."_tb_item AS i USING (it_code) WHERE bill_code = '{$column['book_doc_ref']}' ORDER BY it_code";
} else if($column['book_doc_type'] == 2) {
	$sql_item[0] = "SELECT it_code, it_model_no, it_desc, odit_qty, odit_remark FROM ".ZKP_SQL."_tb_order JOIN ".ZKP_SQL."_tb_order_item USING (ord_code) JOIN ".ZKP_SQL."_tb_item USING (it_code) WHERE ord_code = '{$column['book_doc_ref']}' ORDER BY it_code";
} else if($column['book_doc_type'] == 3) {
	$sql_item[0] = "SELECT it_code, it_model_no, it_desc, dtit_qty, dtit_remark FROM ".ZKP_SQL."_tb_dt JOIN ".ZKP_SQL."_tb_dt_item USING (dt_code) JOIN ".ZKP_SQL."_tb_item USING (it_code) WHERE dt_code = '{$column['book_doc_ref']}' ORDER BY it_code";
} else if($column['book_doc_type'] == 4) {
	$sql_item[0] = "SELECT it_code, it_model_no, it_desc, dfit_qty, dfit_remark FROM ".ZKP_SQL."_tb_df JOIN ".ZKP_SQL."_tb_df_item USING (df_code) JOIN ".ZKP_SQL."_tb_item USING (it_code) WHERE df_code = '{$column['book_doc_ref']}' ORDER BY it_code";
} else if($column['book_doc_type'] == 5) {
	$sql_item[0] = "SELECT it_code, it_model_no, it_desc, drit_qty, drit_remark FROM ".ZKP_SQL."_tb_dr JOIN ".ZKP_SQL."_tb_dr_item USING (dr_code) JOIN ".ZKP_SQL."_tb_item USING (it_code) WHERE dr_code = '{$column['book_doc_ref']}' ORDER BY it_code";
} else if($column['book_doc_type'] == 6) {
	$sql_item[0] = "SELECT it_code, it_model_no, it_desc, rqit_qty, rqit_remark FROM ".ZKP_SQL."_tb_request JOIN ".ZKP_SQL."_tb_request_item USING (req_code) JOIN ".ZKP_SQL."_tb_item USING (it_code) WHERE rq_code = '{$column['book_doc_ref']}' ORDER BY it_code";
} 
...
<input type="hidden" name="_revision_time" value="<?php echo $column['book_revision_time']?>">
	<table width="100%" class="table_box">
		<tr>
			<td colspan="4"><span class="bar_bl">DO INFORMATION</span></td>
		...
		<tr>
			...
			<td width="25%" colspan="2"><span class="bar"><?php echo $column['book_code'] ?></span></td>
			<th width="15%">DO DATE</th>
			<td><?php echo date('d-M-Y', strtotime($column['book_date'])) ?></td>
			<th width="15%">RECEIVED BY</th>
			<td><?php echo $column['book_received_by'] ?></td>
		</tr>
		<tr>
...
?>
	...
	<span class="bar_bl">WAREHOUSE ITEM LIST</span>
	<table width="100%" class="table_l">
		<tr>
			<th width="7%">CODE</th>
			...
			if(e(idx_ed+i*numInput).value=='t') {
				...
				if(booked_qty > 0) {
					...
					for (var j=0; j<countII; j++) {
						if(code==e(idx_codeII+j*numInputII).value) {
