	...
	if ($version == 'v1')
	{
		switch ($source) {
			...
			if($arr['out_doc_type'] == 1) {
				$sql = "SELECT it_code, i.it_model_no, i.it_desc, biit_qty, biit_remark FROM ".ZKP_SQL."_tb_billing JOIN ".ZKP_SQL."_tb_billing_item USING (bill_code) JOIN ".ZKP_SQL."_tb_item AS i USING (it_code) WHERE bill_code = '{$arr['out_doc_ref']}' ORDER BY it_code";
			} else if($arr['out_doc_type'] == 2) {
				$sql = "SELECT it_code, it_model_no, it_desc, odit_qty, odit_remark FROM ".ZKP_SQL."_tb_order JOIN ".ZKP_SQL."_tb_order_item USING (ord_code) JOIN ".ZKP_SQL."_tb_item USING (it_code) WHERE ord_code = '{$arr['out_doc_ref']}' ORDER BY it_code";
			} else if($arr['out_doc_type'] == 3) {
				$sql = "SELECT it_code, it_model_no, it_desc, dtit_qty, dtit_remark FROM ".ZKP_SQL."_tb_dt JOIN ".ZKP_SQL."_tb_dt_item USING (dt_code) JOIN ".ZKP_SQL."_tb_item USING (it_code) WHERE dt_code = '{$arr['out_doc_ref']}' ORDER BY it_code";
			} else if($arr['out_doc_type'] == 4) {
				$sql = "SELECT it_code, it_model_no, it_desc, dfit_qty, dfit_remark FROM ".ZKP_SQL."_tb_df JOIN ".ZKP_SQL."_tb_df_item USING (df_code) JOIN ".ZKP_SQL."_tb_item USING (it_code) WHERE df_code = '{$arr['out_doc_ref']}' ORDER BY it_code";
			} else if($arr['out_doc_type'] == 5) {
				$sql = "SELECT it_code, it_model_no, it_desc, drit_qty, drit_remark FROM ".ZKP_SQL."_tb_dr JOIN ".ZKP_SQL."_tb_dr_item USING (dr_code) JOIN ".ZKP_SQL."_tb_item USING (it_code) WHERE dr_code = '{$arr['out_doc_ref']}' ORDER BY it_code";
			} else if($arr['out_doc_type'] == 6) {
				$sql = "SELECT it_code, it_model_no, it_desc, rqit_qty, rqit_remark FROM ".ZKP_SQL."_tb_request JOIN ".ZKP_SQL."_tb_request_item USING (req_code) JOIN ".ZKP_SQL."_tb_item USING (it_code) WHERE req_code = '{$arr['out_doc_ref']}' ORDER BY it_code";
			} 
			$col = query($sql);
			break;
		    case 'book_item':
