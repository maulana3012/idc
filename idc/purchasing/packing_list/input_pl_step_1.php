...
function fillPO() {
	...
	var sp_code = window.document.frmInsert._sp_code.value;

	if(sp_code.length <= 0) {
		alert("You have to fill supplier code first");
		window.document.frmInsert._sp_code.focus();
	}

	var win = window.open(
		'./p_list_po.php?_code='+sp_code,
		'',
...
[<font color="#446fbe"><?php echo strtoupper($currentDept) ?></font>] NEW PL (STEP 1 / 2)<br />
</strong>
<small class="comment">* Issue PL source by PO</small>
<hr><br />
<form name='frmInsert' method='POST' action="./input_pl_step_2.php">
<input type='hidden' name='p_mode' value='info_pl'>
<input type='hidden' name='_layout_type'>
	...
	<table width="100%" class="table_box" cellspacing="1">
		...
		<tr>
			...
			<th><a href="javascript:fillPO()">PO NO</a></th>
			<td><input name="_po_code" type="text" class="req" size="15" maxlength="64" onclick="javascript:fillPO()" readonly></td>
			<th>PO DATE</th>
			<td><input name="_po_date" type="text" class="reqd" size="15" maxlength="64" readonly></td>
		</tr>
		<tr>
			<th>PO TYPE</th>
			<td>
				<input type="hidden" name="_pl_type">
				<input type="radio" name="_type" value="1" id="a" checked><label for="a">NORMAL</label> &nbsp;
				<input type="radio" name="_type" value="2" id="b"><label for="b">DOOR TO DOOR</label>
			</td>
			<th>SHIPMENT MODE</th>
			<td>
