...
//DEFAULT PROCESS
$col[0] = getPLDetail($version, 'PL', $_code, $_inpl_idx, 'info'); 
$col[1] = getPLDetail($version, 'PL', $_code, $_inpl_idx, 'item'); 
$col[2] = getPLDetail($version, 'PL', $_code, $_inpl_idx, 'item_ed'); 
...
<table width="100%" class="table_box">
	<tr>
		...
		<td colspan="2" align="right"><i>Inputed by: <?php echo $col[0]["inpl_created_by_account"] . date(', j-M-Y g:i:s', strtotime($col[0]["inpl_created_timestamp"]))?></i></td>
	</tr>
	<tr>
		<th width="12%">INVOICE NO</th>
		<td width="40%"><span class="bar_bl"><?php echo $col[0]["pl_inv_no"] ?></span></td>
		<th width="15%">INVOICE DATE</th>
		<td><?php echo date('j-M-Y', strtotime($col[0]["pl_inv_date"]))?></td>
	</tr>
	<tr>
		<th>ETD DATE</th>
		<td><?php echo date('j-M-Y', strtotime($col[0]["pl_etd_date"])) ?></td>
		<th>ETA DATE</th>
		<td><?php echo date('j-M-Y', strtotime($col[0]["pl_eta_date"])) ?></td>
	</tr>
	<tr>
		<th width="15%">RECEIVED BY</th>
		<td><?php echo $col[0]["pl_received_by"]?></td>
	</tr>
	<tr>
		<th>SUPPLIER NAME</th>
		<td colspan="3"><?php echo $col[0]['pl_sp_name']?></td>
	</tr>
	<tr>
		<th width="15%">PL TYPE</th>
		<td>
			<input type="radio" name="_shipment_type" value="1" <?php echo ($col[0]["pl_type"] == 1) ? 'checked' : '' ?> disabled>NORMAL &nbsp;
			<input type="radio" name="_shipment_type" value="2" <?php echo ($col[0]["pl_type"] == 2) ? 'checked' : '' ?> disabled>DOOR TO DOOR
		</td>
		<th>SHIPMENT MODE</th>
		<td>
			<input type="radio" name="_mode" value="sea" <?php echo (trim($col[0]["pl_shipment_mode"]) == 'sea') ? 'checked' : '' ?> disabled>SEA &nbsp;
			<input type="radio" name="_mode" value="air" <?php echo (trim($col[0]["pl_shipment_mode"]) == 'air') ? 'checked' : '' ?> disabled>AIR &nbsp;
			<input type="radio" name="_mode" value="other" <?php echo (trim($col[0]["pl_shipment_mode"]) == 'other') ? 'checked' : '' ?> disabled>OTHER
			<input type="text" name="_mode_desc" class="fmt" size="10" maxlength="15" value="<?php echo $col[0]["pl_shipment_desc"] ?>" readonly>
		</td>
	</tr>
</table>
...
<table width="100%" class="table_box">
	<tr>
		...
		<td width="40%"><?php echo date("j-M-Y", strtotime($col[0]["inpl_checked_date"])) ?></td>
		<th width="15%">CHECKED BY</th>
		<td><?php echo $col[0]["inpl_checked_by"] ?></td>
	</tr>
	<tr>
		<th>WAREHOUSE</th>
		<td colspan="3">
			<input type="radio" name="_warehouse_name" value="1" disabled <?php echo ($col[0]["inpl_warehouse"] == 1) ? 'checked' : '' ?>> INDOCORE &nbsp;
			<input type="radio" name="_warehouse_name" value="2" disabled <?php echo ($col[0]["inpl_warehouse"] == 2) ? 'checked' : '' ?>> DNR
		</td>
	</tr>
	<tr>
		<th>REMARK</th>
		<td colspan="3"><textarea name="_remark" style="width:100%" rows="3" readonly><?php echo $col[0]["inpl_remark"] ?></textarea></td>
	</tr>
</table><br />
