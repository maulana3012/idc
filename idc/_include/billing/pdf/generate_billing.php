...
SELECT
	...
	(select bill_vat_inv_no from " . ZKP_SQL . "_tb_billing where bill_code='$_code') AS fk_no,				--5
	(select cus_type_of_biz from " . ZKP_SQL . "_tb_customer where cus_code='$_pajak_to') AS npwp			--6
";
...
if (ZKP_SQL == 'IDC') {
    if ($_paper_format == "A") {
        if ($currentDept == 'apotik') {
            $tpl_pdf[0] = "tpl_invoice_idc_a_c.pdf";
        } else {
            $tpl_pdf[0] = "tpl_invoice_idc_a_b.pdf";
        }
    } else if ($_paper_format == "B") {
        $tpl_pdf[0] = "tpl_invoice_idc_b.pdf";
    }

    $tpl_pdf[2] = "tpl_do_sj_idc.pdf";
    $tpl_pdf[3] = "tpl_do_ekspedisi_idc.pdf";
    $tpl_pdf[4] = "tpl_invoice_pajak_sederhana.pdf";
    include "generate_billing_a.php";
} else if (ZKP_SQL == 'MED') {
    if ($_paper_format == "A") {
        $tpl_pdf[0] = "tpl_invoice_med_a.pdf";
    } else if ($_paper_format == "B") {
        $tpl_pdf[0] = "tpl_invoice_med_b.pdf";
    }

    $tpl_pdf[2] = "tpl_do_sj_med.pdf";
    $tpl_pdf[3] = "tpl_do_ekspedisi_med.pdf";
    include "generate_billing_b.php";
}
