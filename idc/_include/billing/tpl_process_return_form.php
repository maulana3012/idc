...
if(ckperm(ZKP_INSERT, HTTP_DIR . "$currentDept/$moduleDept/index.php", 'insert')) {
	...
	$_signature_pajak_by	= $_POST['_signature_pajak_by'];
...
//CONFIRM FIX RETURN ============================================================================================
if (ckperm(ZKP_INSERT, HTTP_DIR . "$currentDept/$moduleDept/index.php", 'cfm_return_only')) {
	...
	if (isZKError($result)) {
		$errMessage = $result->getMessage();
		if(strpos($errMessage, "ERR_TYPE_NONVAT_RETURN")) {
			$result = new ZKError(
				"ERR_TYPE_NONVAT_RETURN",
				"ERR_TYPE_NONVAT_RETURN",
				"This is not a vat return invoice!");
		} else if(strpos($errMessage, "ERR_TYPE_RETURN")) {
			$result = new ZKError(
				"ERR_TYPE_RETURN",
				"ERR_TYPE_RETURN",
				"This is not a no only return invoice!");
		}
		$M->goErrorPage($result,  HTTP_DIR . "$currentDept/$moduleDept/index.php");
	}
	goPage(HTTP_DIR . "$currentDept/$moduleDept/revise_return.php?_code=".urlencode($_code));
}

//UNCONFIRM FIX RETURN ============================================================================================
if (ckperm(ZKP_DELETE, HTTP_DIR . "$currentDept/$moduleDept/index.php", 'uncfm_return_only')) {

	$_code		= $_POST['_code'];

	$result = executeSP(
		ZKP_SQL."_uncfmReturnBillingOnly",
		"$\${$_code}$\$"
	);
...
if(ckperm(ZKP_INSERT, HTTP_DIR . "$currentDept/$moduleDept/index.php", 'upload_file')) {
	...
	$M->goPage("revise_return.php?_code=".$_code);
...
if(ckperm(ZKP_DELETE, HTTP_DIR . "$currentDept/$moduleDept/index.php", 'upload_file_delete')) {
	...
	$sql = "DELETE FROM ".ZKP_SQL."_tb_return_file WHERE turn_code='$_code' AND turnf_idx=$_idx ";

	if(isZKError($result =& query($sql)))
	{
		$M->goErrorPage($result, "revise_return.php?_code=".$_code);
	}
