...
<table width="100%" class="table_nn">
	<tr>
		<td height="35"><img src="../../_images/icon/star.gif">&nbsp;&nbsp;<strong>WAREHOUSE INFORMATION</strong> &nbsp; <i>( <a href="revise_billing_2.php?_code=<?php echo $_code ?>">Revised item</a> )</i></td>
		<td align="right">
...
<?php while($items =& fetchRow($cusitem_res)) { ?>
		<tr id="<?php echo trim($items[0])?>">
			...
			<?php if($S->getValue("ma_authority") & 16) { ?>
			...
			<?php } else { ?>
			<td align="right"><input type="text" name="_cus_it_unit_price[]" class="reqn" style="width:100%" value="<?php echo number_format((double)$items[5])?>" onBlur="updateAmount()" onKeyUp="formatNumber(this,'dot')"<?php echo ($column['bill_cus_to']=='6IDC') ? " readOnly":"" ?> readOnly></td>
			<?php } ?>
