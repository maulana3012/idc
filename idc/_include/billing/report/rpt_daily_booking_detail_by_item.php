...
if ($_last_category != 0) {
    ...
    $tmp1[] = "icat.icat_midx IN (" . (empty($catList[0]) ? "0" : $catList[0]) . ")";
    $tmp2[] = "icat.icat_midx IN (" . (empty($catList[0]) ? "0" : $catList[0]) . ")";
}
...
$sql = "$sql1 UNION $sql2 ORDER BY icat_pidx, icat_midx, it_code, book_date, it_idx";
	...
	foreach($group1 as $total2 => $group2) {
		...
		foreach($group2 as $total3 => $group3) {
			...
			foreach($group3 as $total4 => $group4) {
				...
				cell(number_format((double)$rd[$rdIdx][11],2), ' align="right"'); //qty
