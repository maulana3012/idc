...
$_order_by		= isset($_REQUEST['_order_by']) ? $_REQUEST['_order_by'] : "";
...
function confirmValue(idx) {
	...
	if(target == 'customer') {
		...
		f._cus_npwp.value		= cus[idx][3];
		f._cug_code.value		= cus[idx][4];
		f._cus_to_responsible_by.value	= cus[idx][5];
	} else if (target == 'ship') {
		f._ship_to.value		= cus[idx][0];
		f._ship_name.value		= cus[idx][1];
		f._ship_to_responsible_by.value	= cus[idx][5];
	} else if(target == 'pajak') {
		f._pajak_to.value		= cus[idx][0];
		f._pajak_name.value		= cus[idx][1];
		f._pajak_address.value	= cus[idx][2];
	} else if(target == 'deposit') {
		var f = window.opener.document.frmDeposit;
    ...
    // Billing Summary
        ...
        f.submit();
    } else if(target == 'summary_monthly') {
        var f = window.opener.document.frmSearch;
        f._cus_code.value       = cus[idx][0];
        f.submit();
    }    
...
<table width="100%" class="table_box">
	<tr>
		...
		<th rowspan="2" width="15%">
			...
			<a href="javascript:document.location.href='p_list_cus_code.php?_cus_channel=<?php echo $_cus_channel ?>&_order_by=<?php echo $_order_by ?>'"><img src="../../_images/icon/list_mini.gif" alt="see all"></a>
		</th>
	</tr>
	<tr>
...
<script language="javascript" type="text/javascript">
	window.document.frmSearch._cus_channel.onchange = function() {
		window.location.href = "?_cus_channel=" + window.document.frmSearch._cus_channel.value +'&_order_by='+ window.document.frmSearch._order_by.value;
	}
</script>
<table width="100%" class="table_box">
	<tr height="30px">
