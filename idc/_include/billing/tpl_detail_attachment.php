<?php
$file_sql = "SELECT * FROM ".ZKP_SQL."_tb_billing_file WHERE bill_code = '$_code' AND billf_file_type not in ('Faktur Pajak', 'Faktur Pajak Rev')";
...
<table width="100%" class="table_c">
	...
	while($items =& fetchRowAssoc($file_res)) {
	    ...
	    echo "
	    <td align='center'>
		<img  src='". USER_DATA . "archieve" . $items['billf_file_path'] ."' width='200px'>
		<a target='_blank' href='". USER_DATA . "archieve" . $items['billf_file_path'] ."'>Download</a> |
		<a href=\"javascript:deleteFile(". $items['billf_idx'] .", '" . $items['billf_file_type'].' - '.$items['billf_file_name'] ."', '".$items['billf_file_path']."')\">Delete</a><br />
		<strong>[ ". $items['billf_file_type'] ." ]</strong> ". $items['billf_file_desc'] ."
	    </td>
	    ";
	    $i++;
	    if( $i == 4 ) { echo "</tr>"; $i=0; }
