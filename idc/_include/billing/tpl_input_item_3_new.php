...
function createItem() {
	...
	var oRows = document.querySelectorAll('#salesPosition tr');
...
function updateQtyBill(indicator, code, price, sl_idx) {
	...
	var oRowsI = document.querySelectorAll('#salesPosition tr');
	var countI		= oRowsI.length;
	var oRowsII = document.querySelectorAll('#billPosition tr');
	var countII		= oRowsII.length;
...
function checkSalesIdx(indicator, idx, start) {
	...
	if(indicator) {		//------------------------------------------------------------------- when item inserted
		...
		for (var i=start; i<countI; i++) {
			var oRowI = oRowsI[i];
			var oCell = oRowI.querySelectorAll('td')
				var col = new Array;
				col[0] = trim(oCell[0].innerText);	// date
				col[1] = trim(oCell[1].innerText);	// cus code
				col[2] = trim(oCell[3].innerText);	// faktur no
				col[3] = trim(oCell[4].innerText);	// LOP no
