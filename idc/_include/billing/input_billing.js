function fillCustomer(target) {
	...
	var win = window.open(
		'../../_include/billing/p_list_cus_code.php?_dept='+dept+'&_check_code='+ keyword + '&_order_by=' + ord_by,
		target,
...
function initBillDept(access) {
	...
	if(access == 'ALL' || access == 'IDC') {
		...
		} else if(dept == 'D' || dept == 'H' || dept == 'M' || dept == 'P' || dept == 'T' || dept == 'S') {
		...
		var dsb_vat = false;
		var chk_btnVat = 0;
		if (f._dept.value == 'A' || f._dept.value == 'H' || f._dept.value == 'T' || f._dept.value == 'P' || f._dept.value == 'S')
		{
			var dsb_btnVat = true;
		} else 
		{
...
function bankDesc(o) {
	...
	if (o == "BCA1") {
		f._bank_address.value = "BCA KCU SCBD \nA/N\t: PT. INDOCORE PERKASA\nA/C\t: 006-3889595";
	} else if (o == "BCA2") {
