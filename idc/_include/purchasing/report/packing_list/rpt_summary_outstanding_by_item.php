...
SELECT
  ...
  ".ZKP_SQL."_arrivedQty(1, pl.pl_idx::varchar, it.it_code) AS it_arrived_qty,
...
WHERE ".ZKP_SQL."_arrivedQty(1, pl.pl_idx::varchar, it.it_code) < plit.plit_qty and $strWherePL" ;
...
SELECT
  ...
  ".ZKP_SQL."_arrivedQty(2, cl.cl_idx::varchar, it.it_code) AS it_arrived_qty,
...
FROM
  ...
  JOIN ".ZKP_SQL."_tb_item_cat AS icat USING(icat_midx)
  JOIN ".ZKP_SQL."_tb_item AS it USING(it_code)
WHERE ".ZKP_SQL."_arrivedQty(2, cl.cl_idx::varchar, it.it_code) < clit.clit_qty and  $strWhereClaim ";

$sql = "$sql_pl UNION $sql_claim ORDER BY icat_pidx, icat_midx, it_code, invoice_date";
//echo $sql_claim;
// raw data
$rd = array();
$rdIdx = 0;
...
END;
	...
	foreach($group1 as $total2 => $group2) {
		...
		print "<tr>\n";
		cell($item, ' colspan="3" align="right" style="color:darkblue"');
