	...
	foreach($group1 as $total2 => $group2) {
		...
		foreach($group2 as $total3 => $group3) {
			...
			if($it_code=='2101' || $it_code=='2101NE' || $it_code=='2200') {
				...
				$status = ($rd[$rdIdx][4]<0) ? 'expired':'available';
			}
			...
			cell(number_format($rd[$rdIdx][5],2), ' style="'.$display_css[$status].'" valign="top" align="right"');	//vat
			cell(number_format($rd[$rdIdx][6],2), ' style="'.$display_css[$status].'" valign="top" align="right"');	//non vat
			cell(number_format($rd[$rdIdx][7],2), ' style="'.$display_css[$status].'" valign="top" align="right"');	//qty
			if($status=='expired') {
				print "\t<td align=\"center\"><a href=\"javascript:deleteItem('".trim($rd[$rdIdx][1])."','{$rd[$rdIdx][3]}')\" style=\"font-family:MS Mincho;background-color:lightyellow\"> &nbsp;delete </a></td>\n"; 
			}
			print "</tr>\n";
...
print <<<END
	<table width="73%" class="table_box">
		<tr>
			<th rowspan="2" width="15%">MODEL NO</th>
			<th rowspan="2" width="10%">EXPIRED DATE</th>
			<th rowspan="2" width="5%">M/S<br />(month/s)</th>
...
cell(number_format($gg_total[0],2), ' align="right" style="color:brown; background-color:lightyellow"');
cell(number_format($gg_total[1],2), ' align="right" style="color:brown; background-color:lightyellow"');
