...
SELECT
 ...
 ".ZKP_SQL."_lastArrived('PO', 'PL', 'Invoice', po.po_code, it.it_code) AS last_arrived_inv,
 ".ZKP_SQL."_lastArrived('PO', 'PL', 'Date', po.po_code, it.it_code) AS last_arrived_date
FROM
...
WHERE " . $strWhere .  "
GROUP BY
 po.po_code, po.po_date, status, sp.sp_code, sp.sp_full_name, it.it_code, it.it_model_no
ORDER BY
 sp.sp_code, po.po_code, it.it_code
";
echo "<pre>";
//var_dump($sql);
echo "</pre>";
...
END;
	...
	foreach($group1 as $total2 => $group2) {
		...
		foreach($group2 as $group3) {
			...
			cell($rd[$rdIdx][8], ' style="'.$display_css[$rd[$rdIdx][4]].'" align="right"');			//RECEIVED QTY
			cell(number_format($rd[$rdIdx][7]-$rd[$rdIdx][8]), ' style="'.$display_css[$rd[$rdIdx][4]].'" align="right"');	//OUTSTANDING QTY
			cell($rd[$rdIdx][9],' style="'.$display_css[$rd[$rdIdx][4]].'" align="center"');								//LAST INVOICE NO
