...
function confirmValue(idx) {
	...
	if(target == 'customer') {
		...
		f._bill_to.value			= cus[idx][0];
		f._bill_to_attn.value		= cus[idx][1];
		f._bill_to_address.value	= cus[idx][2];
	} else if (target == 'ship') {
		f._ship_to.value			= cus[idx][0];
		f._ship_to_attn.value		= cus[idx][1];
		f._ship_to_address.value	= cus[idx][2];
	} else if(target == 'summary_date') {
		...
		f.s_mode.value = 'date';
		f.submit();
	} else if(target == 'summary_period') {
		var f = window.opener.document.frmSearch;
		f._cus_code.value = cus[idx][0];
		f.some_date.value = '';
		f.s_mode.value = 'period';
		f.submit();
	} else if(target == 'consignment_monthly') {
		var f = window.opener.document.frmSearch;
		f._cus_code.value = cus[idx][0];
		f._cug_code.value = '';
		f.submit();
	...
	// Sales Report =================================================
	} else if(target == 'sales_log') {
		var f = window.opener.document.frmSearch;
		f._cus_code.value = cus[idx][0];
		f.some_date.value = '';
		f.s_mode.value = 'period';
		f.submit();
	} else if(target == 'input_sales') {
		f._cus_to.value = cus[idx][0];
		f._cus_to_attn.value = cus[idx][1];
		f._cus_to_address.value = cus[idx][2];
		f._cus_to_responsible_by.value = cus[idx][4];

	// Inventory ====================================================
	} else if(target == 'summary_inv') {
		f._cus_to.value = cus[idx][0];
		f._cus_to_attn.value = cus[idx][1];
		f._cus_to_address.value = cus[idx][2];
		f.submit();
	} else if(target == 'summary_inv_setup') {
		f._cus_to.value = cus[idx][0];
		f._cus_to_attn.value = cus[idx][1];
		f._cus_to_address.value = cus[idx][2];
	} 
...
function initPage() {
	window.frmSearch._check_code.focus();
	if(window.document.frmSearch._dept.value == '') {
