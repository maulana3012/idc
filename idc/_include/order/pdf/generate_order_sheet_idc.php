...
/*	2	*/	$pdf->setXY(30,48);
			$pdf->Cell(103, 4, $_cus_to_attn);
			$pdf->Cell(45, 4, $_cus_to);
			$pdf->Cell(0, 4, $_code);
/*	3	*/	$pdf->setXY(30,52);
			$pdf->Cell(148, 4, $_cus_to_address);
			$pdf->Cell(0, 4, $_po_date);
/*	4	*/	$pdf->setXY(30,58);
			$pdf->Cell(103, 4, $_ship_to_attn);
			$pdf->Cell(45, 4, $_ship_to);
/*	5	*/	$pdf->setXY(30,63);
			$pdf->Cell(148, 4, $_ship_to_address);
/*	6	*/	$pdf->setXY(30,68);
			$pdf->Cell(103, 4, $_po_no);
...
if($counter == 0) {
	...
	$pdf->setXY(10, 83);
	$pdf->Cell(117,4, 'Previous balance',0);
...
IF($_dept == 'A') {
/*	1A	*/	$pdf->setXY(31,202);
			$pdf->Cell(7, 4, $_price_discount, 0, 0, 'R'); // Discount
			$pdf->Cell(38);
			$pdf->Cell(6, 4, ($_price_chk & 1) ? "X":""); // Dealer 1's
			$pdf->Cell(35);
			$pdf->Cell(6, 4, ($_price_chk & 2) ? "X":""); // Dealer 2's
			$pdf->Cell(23);
			$pdf->Cell(6, 4, ($_price_chk & 4) ? "X":""); // Retailer's
			$pdf->setXY(146, 206);
			$pdf->Cell(6, 4, ($_price_chk & 8) ? "X":""); // Consumer's
}
/*	1B	*/	$pdf->setXY(34,213);
...
/*	5	*/	$pdf->setXY(39, 229);
			...
			$pdf->Cell(30, 4, substr($_payment_transfer_by, 0, 18));	// transfer by
			$pdf->Cell(40, 4, substr($_payment_giro_by, 0, 18));		// giro by
/*	6	*/	$pdf->setXY(10, 239);
			$pdf->MultiCell(190, 4, $_remark);							// Remark
