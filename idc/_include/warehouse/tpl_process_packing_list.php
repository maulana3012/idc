...
if(ckperm(ZKP_INSERT, HTTP_DIR . "$currentDept/$moduleDept/index.php", 'confirm_PL')) {
	...
	//new Incoming PL from PO
	$result = executeSP(
		ZKP_SQL."_PLImportInsert",
		$_pl_idx,
		"$\${$_sp_code}$\$",
		"$\${$_po_code}$\$",
		$_pl_type,
	...
	$_inpl_idx = $result[0];
	$M->goPage(HTTP_DIR . "$currentDept/$moduleDept/detail_confirm_pl.php?_ver=v2&_code=$_code&_inpl_idx=$_inpl_idx");
}
...
if(ckperm(ZKP_UPDATE, HTTP_DIR . "$currentDept/$moduleDept/index.php", 'update_PL')) {
	...
	$_rcp = get_diff_data(array($_POST['_it_code'], $_POST['_it_qty'], $_POST['_in_it_code'], $_POST['_in_it_qty']));
/*
echo "<pre>";
var_dump($_it_code, $_rcp['item'], $_rcp['qty']);
echo "</pre>";
exit;
*/
	//modify incoming PL to indocore
	$result = executeSP(
		ZKP_SQL."_PLImportUpdate",
		$_pl_idx,
		$_inpl_idx,
		$_wh_location,
		$_inpl_type,
		...
		"$\${$_remark}$\$",
		"ARRAY[$_rcp_idx]",
		"ARRAY[$_it_code]", "ARRAY[$_it_qty]",
		"ARRAY[$_ed_it_code]", "ARRAY[$_ed_it_qty]", "ARRAY[$_ed_it_date]",
...
if(ckperm(ZKP_INSERT, HTTP_DIR . "$currentDept/$moduleDept/index.php", 'confirm_PL_local')) {
	...
	$_inlc_idx = $result[0];
	$M->goPage(HTTP_DIR . "$currentDept/$moduleDept/detail_confirm_pl_local.php?_ver=v2&_code=$_inlc_idx");
}
...
if(ckperm(ZKP_UPDATE, HTTP_DIR . "$currentDept/$moduleDept/index.php", 'update_PL_local')) {
	...
	if (isZKError($result)) {
		$M->goErrorPage($result,  HTTP_DIR . "$currentDept/$moduleDept/revise_confirm_pl_local.php?_ver=v2&_inlc_idx=$_inlc_idx");
	}
...
if(ckperm(ZKP_INSERT, HTTP_DIR . "$currentDept/$moduleDept/index.php", 'confirm_claim')) {
	...
	$_incl_idx = $result[0];
	$M->goPage(HTTP_DIR . "$currentDept/$moduleDept/detail_confirm_claim.php?_ver=v2&_code=$_code&_incl_idx=$_incl_idx");
}
