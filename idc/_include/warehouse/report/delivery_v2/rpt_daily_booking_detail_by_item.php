...
if($_order_by == '1') {
	$tmp[] = "substr(book_code,1,1)='D' and substr(book_code,4,1)!='M'";
} else if($_order_by == '2') {
	$tmp[] = "substr(book_code,1,1)='D' and substr(book_code,4,1)='M'";
}
...
if($_dept != 'all') {
	if($_dept == 'DEMO') {
		...
		$tmp[] = "b.book_dept = '$_dept' AND b.book_doc_type != 6";
	}
}
...
SELECT
  ...
  CASE
 	WHEN book_doc_type = 6 AND book_is_revised=false THEN 'confirm_request.php?_code='||b.book_idx
 	WHEN book_doc_type != 6 AND book_is_revised=false THEN 'confirm_do.php?_code='||b.book_idx
	WHEN book_is_revised=true THEN 'detail_do.php?_code='|| (SELECT out_idx FROM ".ZKP_SQL."_tb_outgoing_v2 WHERE out_doc_ref= b.book_doc_ref ) || '&_source=v2'
  END AS go_page
FROM
...
END;
	...
	foreach($group1 as $total2 => $group2) {
		...
		foreach($group2 as $total3 => $group3) {
			...
			cell("[". trim($rd[$rdIdx][8]) ."] ".$rd[$rdIdx][9], ' valign="top" rowspan="'.$rowSpan.'"');	//Customer name
