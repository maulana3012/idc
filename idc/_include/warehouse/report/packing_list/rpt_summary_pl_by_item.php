...
SELECT
  ...
  (select sum(plit_qty) from ".ZKP_SQL."_tb_pl_local_item where it_code=it.it_code and po_code=pl.po_code and pl_no=pl.pl_no) AS total_qty,
  inlc_idx AS inpl_idx,
  to_char(inlc.inlc_checked_date, 'dd-Mon-yy') AS arrival_date,
  init.init_qty AS init_qty,
...
$sql = "$sql_pl UNION $sql_claim UNION $sql_local ORDER BY icat_pidx, icat_midx, it_code, invoice_date, idx, sp_code";
