...
$_order_by	= isset($_GET['cboOrderBy']) ? $_GET['cboOrderBy'] : $cboFilter[1][ZKP_FUNCTION][0][0];
...
<table width="100%" class="table_layout">
	<tr>
		<td width="70%" rowspan="2"><strong class="title">[<font color="#446fbe"><?php echo strtoupper($currentDept) ?></font>] BOOKING SUMMARY by group<br /><br /></strong></td>
		<td>CATEGORY</td>
	</tr>
	<tr>
		<td>
...
<table width="100%" class="table_box">
	<tr>
		<td align="right">
			<a href="./daily_booking_by_item.php?<?php echo getQueryString()?>">View By Item</a>
		</td>
	</tr>
</table>
<?php require_once APP_DIR . "_include/billing/report/rpt_daily_booking_detail_by_group.php" ?>
<script language="javascript1.2" type="text/javascript">
	var f = window.document.frmSearch;
	var ts = <?php echo time() * 1000;?>;

	<?php if($currentDept=='apotik') { ?> setSelect(f._cug_code, "<?php echo isset($_GET['_cug_code']) ? $_GET['_cug_code'] : "all"?>"); <?php } ?>
	setSelect(f.cboSource, "<?php echo isset($_GET['cboSource']) ? $_GET['cboSource'] : "all"?>");
	setSelect(f.cboOrderBy, "<?php echo isset($_GET['cboOrderBy']) ? $_GET['cboOrderBy'] : $cboFilter[1][ZKP_FUNCTION][0][0]?>");
...
?>
	...
	f.cboOrderBy.onchange	= f.cboSource.onchange;
	<?php if($currentDept=='apotik') { ?> f._cug_code.onchange	= f.cboSource.onchange; <?php } ?>
	...
	function summarizeDO() {
		...
		var win = window.open(
			'./p_summary_outstanding_do.php?_code='+ keyword,
			'',
