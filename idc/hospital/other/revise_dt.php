...
//========================================================================================= DEFAULT PROCESS
//dt
$sql	= "SELECT *,(SELECT book_idx FROM ".ZKP_SQL."_tb_booking WHERE book_doc_ref='$_code' AND book_doc_type=3) AS book_idx FROM ".ZKP_SQL."_tb_dt WHERE dt_code = '$_code'";
$result =& query($sql);
...
if(numQueryRows($result) <= 0) {
	...
	goPage(HTTP_DIR . "$currentDept/$moduleDept/detail_dt.php?_code=".urlencode($column['dt_code']));
}
  ...
  <tr>
    <td style="padding:0 3 3 3">
        ...
        <tr>
          	...
          	<!--START: BODY-->
<strong class="title">[<font color="#446fbe"><?php echo strtoupper($currentDept) ?></font>] REVISE RETURN DO TEMPORARY</strong><br /><br />
<form name='frmInsert' method='POST'>
<input type='hidden' name='p_mode'>
<input type='hidden' name='_code' value="<?php echo $column['dt_code'] ?>">
<input type="hidden" name="_dept" value="<?php echo $column['dt_dept']?>">
<input type='hidden' name="_revision_time" value="<?php echo $column['dt_revesion_time']?>">
...
<input type='hidden' name='_date' value="<?php echo $column['dt_date'] ?>">
<table width="100%" class="table_box">
	...
	<tr>
		...
		<td><input type="text" name="_do_date" class="reqd" size="15" value="<?php echo date('j-M-Y', strtotime($column['dt_date'])) ?>"></td>
	</tr>
	<tr>
		<th>ISSUED BY</th>
		<td><input type="text" name="_received_by" class="req" size="20" maxlength="32" value="<?php echo $column['dt_received_by'] ?>"></td>
		<th>ISSUED DATE</th>
		<td><input type="text" name="_issued_date" class="fmtd" size="15" value="<?php echo ($column['dt_issued_date']=='')?'':date('j-M-Y', strtotime($column['dt_issued_date'])) ?>"></td>
	</tr>
	<tr>
		<th>REQUEST BY</th>
		<td><input type="text" name="_issued_by" class="fmt" size="20" maxlength="32" value="<?php echo $column['dt_issued_by'] ?>"></td>
		<th>TYPE ITEM</th>
		<td>
...
<table class="table_box" width="100%">
	<tr>
		...
		<td>1.<input type="text" name="_delivery_warehouse" class="fmt" size="2" maxlength="2" value="<?php echo $column['dt_delivery_warehouse'] ?>">ex W/house(P/C/D)</td>
		<td>2.<input type="text" name="_delivery_franco" class="fmt" size="2" maxlength="2" value="<?php echo $column['dt_delivery_franco'] ?>">Franco(P/D)</td>
		<td>by <input type="text" name="_delivery_by" size="6" class="fmt" value="<?php echo $column['dt_delivery_by'] ?>"></td>
		<td>Freight charge : Rp <input type="text" name="_delivery_freight_charge" class="fmtn" onKeyUp="formatNumber(this,'dot')" size="8" onBlur="updateAmount()" value="<?php echo number_format((double)$column['dt_delivery_freight_charge']) ?>"></td>
	</tr>
	<tr>
		<th>REMARK</th>
		<td colspan="4"><textarea name="_remark" style="width:100%" rows="4"><?php echo $column['dt_remark'] ?></textarea></td>
	</tr>
</table>
</form>
<table width="100%" cellpadding="2" cellspacing="0">
	<tr>
		<td>
			<button name='btnDelete' class='input_red' style='width:120px;'><img src="../../_images/icon/trash.gif" width="15px" align="middle" alt="Delete DO"> &nbsp; Delete DO</button>
		</td>
		<td align="right">
			Rev No:
...
<script language="javascript" type="text/javascript">
	...
	window.document.all.btnDelete.onclick = function() {
		if(confirm("Are you sure to delete billing?\n Deleting DT will delete booking items too")) {
			oForm.p_mode.value = 'delete_dt';
			oForm.submit();
		}
	}
