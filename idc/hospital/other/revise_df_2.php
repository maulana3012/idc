...
//df
$sql	= "SELECT *,
	(SELECT book_idx FROM ".ZKP_SQL."_tb_booking WHERE book_doc_ref='$_code' AND book_doc_type=4) AS book_idx,
	(SELECT book_is_revised FROM ".ZKP_SQL."_tb_booking WHERE book_doc_ref='$_code' AND book_doc_type=4) AS book_is_revised
	FROM ".ZKP_SQL."_tb_df WHERE df_code = '$_code'";
$result =& query($sql);
...
if(numQueryRows($result) <= 0) {
	...
	$message = new ZKError(
		...
		"HAS_NOT_CONFIRMED_BY_WAREHOUSE",
		"DF no $_code has not been confirmed by warehouse. Please check again");
	$M->goErrorPage($message,  HTTP_DIR . "$currentDept/$moduleDept/revise_df.php?_code=".urlencode($_code));
  ...
  <tr>
    <td style="padding:0 3 3 3">
        ...
        <tr>
          	...
          	<!--START: BODY-->
<strong class="title">[<font color="#446fbe"><?php echo strtoupper($currentDept) ?></font>] REVISE DO FREE</strong><br /><br />
<form name='frmInsert' method='POST'>
<input type='hidden' name='p_mode'>
<input type='hidden' name='_code' value="<?php echo $column['df_code'] ?>">
<input type="hidden" name="_dept" value="<?php echo $column['df_dept']?>">
<input type='hidden' name="_revision_time" value="<?php echo $column['df_revesion_time']?>">
<input type="hidden" name="_book_idx" value="<?php echo $column['book_idx']?>">
<input type="hidden" name="_type_item" value="<?php echo $column['df_type_item']?>">
<input type='hidden' name='_date' value="<?php echo $column['df_date'] ?>">
<table width="100%" class="table_box">
	...
	<tr>
		...
		<td><input type="text" name="_do_date" class="reqd" size="15" value="<?php echo date('j-M-Y', strtotime($column['df_date'])) ?>"></td>
	</tr>
	<tr>
		<th>ISSUED BY</th>
		<td><input type="text" name="_received_by" class="req" size="20" maxlength="32" value="<?php echo $column['df_received_by'] ?>"></td>
		<th>ISSUED DATE</th>
		<td><input type="text" name="_issued_date" class="fmtd" size="15" value="<?php echo ($column['df_issued_date']=='')?'':date('j-M-Y', strtotime($column['df_issued_date'])) ?>"></td>
	</tr>
	<tr>
		<th>REQUEST BY</th>
		<td><input type="text" name="_issued_by" class="fmt" size="20" maxlength="32" value="<?php echo $column['df_issued_by'] ?>"></td>
		<th>TYPE ITEM</th>
		<td>
...
<table width="100%" class="table_box" cellspacing="1">
	<tr>
		...
		<td width="20%"><input type="text" name="_cus_to" class="req" size="5" value="<?php echo $column['df_cus_to'] ?>"></td>
		<th width="12%">NAME</th>
		<td><input type="text" name="_cus_name" class="fmt" style="width:100%" value="<?php echo $column['df_cus_name'] ?>"></td>
	</tr>
	<tr>
		<th>ADDRESS</th>
		<td colspan="3"><input type="text" name="_cus_address" class="fmt"  style="width:100%" maxlength="255" value="<?php echo $column['df_cus_address'] ?>"></td>
	</tr>
	<tr>
		<th>SHIP TO</th>
		<th><a href="javascript:fillCustomer('ship')">CODE</a></th>
		<td><input type="text" name="_ship_to" class="req" size="5" value="<?php echo $column['df_ship_to'] ?>"></td>
		<th>NAME</th>
		<td><input type="text" name="_ship_name" class="fmt" style="width:100%" value="<?php echo $column['df_ship_name'] ?>"></td>
	</tr>
</table><br />
...
<table class="table_box" width="100%">
	<tr>
		...
		<td>1.<input type="text" name="_delivery_warehouse" class="fmt" size="2" maxlength="2" value="<?php echo $column['df_delivery_warehouse'] ?>">ex W/house(P/C/D)</td>
		<td>2.<input type="text" name="_delivery_franco" class="fmt" size="2" maxlength="2" value="<?php echo $column['df_delivery_franco'] ?>">Franco(P/D)</td>
		<td>by <input type="text" name="_delivery_by" size="6" class="fmt" value="<?php echo $column['df_delivery_by'] ?>"></td>
		<td>Freight charge : Rp <input type="text" name="_delivery_freight_charge" class="fmtn" onKeyUp="formatNumber(this,'dot')" size="8" onBlur="updateAmount()" value="<?php echo number_format((double)$column['df_delivery_freight_charge']) ?>"></td>
	</tr>
	<tr>
		<th>REMARK</th>
		<td colspan="4"><textarea name="_remark" style="width:100%" rows="4"><?php echo $column['df_remark'] ?></textarea></td>
	</tr>
</table>
<?php
...
<script language="javascript" type="text/javascript">
	...
	window.document.all.btnUpdate.onclick = function() {
		...
		if(confirm("Are you sure to update?")) {
			if(verify(oForm)){
				oForm.p_mode.value = 'update_df_revised';
				oForm.submit();
			}
		}
	}
	...
	window.document.all.btnCancel.onclick = function() {
		window.location.href = 'revise_df.php?_code=<?php echo $_code ?>';
	}
</script>
            <!--END: BODY-->
          </td>
